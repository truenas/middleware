...
@pytest.mark.parametrize('command,name,ttl,rtype,rdata,expected', [
    ('ADD', FQDN, 3600, 'A', '192.168.1.5', 'update add truenas.ad.example.com. 3600 A 192.168.1.5 \n'),
    ('DELETE', FQDN, 60, 'AAAA', '2001:db8::5', 'update delete truenas.ad.example.com. 60 AAAA 2001:db8::5 \n'),
    ('ADD', '5.1.168.192.in-addr.arpa', 3600, 'PTR', FQDN,
...
def test_multiple_same_family_addresses_each_isolated():
    ...
    assert [rp for rp, _ in plan.reverse] == ['5.1.168.192.in-addr.arpa', '6.50.168.192.in-addr.arpa']
