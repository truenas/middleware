...
def check_file_idmap(filename, account_xid, target_xid):
    lines = []
    with open(filename, 'r') as f:
        for line in f:
            if target_xid is None:
                assert not line.startswith(f'{account_xid}:')
            elif line.startswith(f'{account_xid}:'):
                src, target, cnt = line.split(':')
                assert int(target) == target_xid
                return

            lines.append(line)

    assert target_xid is None, f'{target_xid}: ID not found in {filename}: {lines}'
...
def test__user_idmap_namespace_create(param):
    with create_user('test_user', userns_idmap=param) as res:
        ...
        check_file_idmap('/etc/subuid', res['uid'], res['uid'] if param == 'DIRECT' else param)
...
def test__user_idmap_namespace_update(user, param):
    with Client() as c:
        ...
        check_file_idmap('/etc/subuid', user['uid'], user['uid'] if param == 'DIRECT' else param)
...
def test__group_idmap_namespace_create(param):
    with create_group('test_group', userns_idmap=param) as res:
        ...
        check_file_idmap('/etc/subgid', res['gid'], res['gid'] if param == 'DIRECT' else param)
