...
def test_write_role_can_write(unprivileged_user_fixture, role):
    ...
    common_checks(
        unprivileged_user_fixture, "service.control", role, True, method_args=["nfs", "START"],
        method_kwargs=dict(job=True), valid_role_exception=False,
    )
    common_checks(
        unprivileged_user_fixture, "service.control", role, True, method_args=["nfs", "RESTART"],
        method_kwargs=dict(job=True), valid_role_exception=False,
    )
    common_checks(
        unprivileged_user_fixture, "service.control", role, True, method_args=["nfs", "RELOAD"],
        method_kwargs=dict(job=True), valid_role_exception=False,
    )
    common_checks(
        unprivileged_user_fixture, "service.control", role, True, method_args=["nfs", "STOP"],
        method_kwargs=dict(job=True), valid_role_exception=False,
    )
