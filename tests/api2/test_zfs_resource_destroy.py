...
def test_zfs_resource_destroy_non_recursive_with_children_fails():
    ...
    root = create_resource("test_fs_parent/child", {"create_ancestors": True})
...
def test_zfs_resource_destroy_recursive_filesystem():
    ...
    result = call("zfs.resource.query", {"paths": [lvl0], "get_children": True, "properties": None})
    assert len(result) == 8
    call("zfs.resource.destroy", {"path": lvl0, "recursive": True})
    result = call("zfs.resource.query", {"paths": [lvl0]})
    assert len(result) == 0
...
def test_zfs_resource_destroy_with_hold():
    ...
    snap = "snap"
    call("zfs.snapshot.create", {"dataset": source, "name": snap})
    call("zfs.snapshot.hold", {"snapshot": f"{source}@{snap}", "tag": "test_hold"})
    ...
    call("zfs.resource.destroy", {"path": f"{source}@{snap}", "remove_holds": True})
