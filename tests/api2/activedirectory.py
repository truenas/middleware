...
@skip_ad_test
def test_12_get_activedirectory_state():
    global results
    results = GET('/activedirectory/get_state/')
    assert results.status_code == 200, results.text
...
@skip_ad_test
def test_13_get_activedirectory_started():
    results = GET('/activedirectory/started/')
    assert results.status_code == 200, results.text
...
@skip_ad_test
def test_14_get_activedirectory_data():
    global results
    results = GET('/activedirectory/')
    assert results.status_code == 200, results.text
...
def test_15_verify_activedirectory_data_of_(data):
    if data == 'domainname':
...
def test_17_verify_the_value_of_put_smb_object_value_of_(data):
    assert results.json()[data] == payload[data], results.text


@skip_ad_test
def test_18_get_smb_data():
    global results
    results = GET("/smb/")
    assert results.status_code == 200, results.text
...
@skip_ad_test
@pytest.mark.parametrize('data', ["description", "guest"])
def test_19_verify_the_value_of_get_smb_object_(data):
    assert results.json()[data] == payload[data], results.text


@skip_ad_test
def test_20_creating_a_smb_share_on_smb_path():
...
def test_21_verify_the_value_of_the_created_sharing_smb_object_(data):
    assert results.json()[data] == payload[data], results.text


@skip_ad_test
def test_22_get_sharing_smb_from_id():
    global results
...
@skip_ad_test
@pytest.mark.parametrize('data', ["comment", "path", "name", "guestok", "vfsobjects"])
def test_23_verify_the_value_of_get_sharing_smb_object_(data):
    assert results.json()[data] == payload[data], results.text


@skip_ad_test
def test_24_enable_cifs_service():
    results = PUT("/service/id/cifs/", {"enable": True})
    assert results.status_code == 200, results.text


@skip_ad_test
def test_25_checking_to_see_if_clif_service_is_enabled():
...
@skip_ad_test
def test_27_checking_to_see_if_nfs_service_is_running():
    results = GET("/service?service=cifs")
...
@bsd_host_cfg
@skip_ad_test
def test_28_creating_smb_mountpoint():
...
@bsd_host_cfg
@skip_ad_test
def test_29_store_AD_credentials_in_a_file_for_mount_smbfs():
...
@bsd_host_cfg
@skip_ad_test
def test_34_deleting_SMB_file_1_2():
...
@bsd_host_cfg
@skip_ad_test
def test_35_deleting_SMB_file_2_2():
...
@bsd_host_cfg
@skip_ad_test
def test_37_removing_SMB_mountpoint():
...
@skip_ad_test
def test_38_leave_activedirectory():
    global payload, results
    payload = {
        "username": ADUSERNAME,
        "password": ADPASSWORD
...
@skip_ad_test
def test_39_get_activedirectory_state():
    results = GET('/activedirectory/get_state/')
    assert results.status_code == 200, results.text
    assert results.json() == 'DISABLED', results.text
...
@skip_ad_test
def test_40_get_activedirectory_started_after_leaving_AD():
    results = GET('/activedirectory/started/')
    assert results.status_code == 200, results.text
    assert results.json() is False, results.text
...
@skip_ad_test
def test_41_re_enable_activedirectory():
    global payload, results
    payload = {
        "bindpw": ADPASSWORD,
        "bindname": ADUSERNAME,
...
@skip_ad_test
def test_43_get_activedirectory_started():
    results = GET('/activedirectory/started/')
    assert results.status_code == 200, results.text
    assert results.json() is True, results.text
...
@skip_ad_test
def test_44_get_activedirectory_data():
    global results
    results = GET('/activedirectory/')
    assert results.status_code == 200, results.text
...
@skip_ad_test
@pytest.mark.parametrize('data', ad_object_list)
def test_45_verify_activedirectory_data_of_(data):
    if data == 'domainname':
        assert results.json()[data].lower() == payload[data], results.text
    else:
        assert results.json()[data] == payload[data], results.text
...
@osx_host_cfg
@skip_ad_test
def test_46_Create_mount_point_for_SMB_on_OSX_system():
...
@osx_host_cfg
@skip_ad_test
def test_47_Mount_SMB_share_on_OSX_system():
...
@osx_host_cfg
@skip_ad_test
def test_48_Create_file_on_SMB_share_via_OSX_to_test_permissions():
...
@osx_host_cfg
@skip_ad_test
def test_49_Moving_SMB_test_file_into_a_new_directory():
...
@osx_host_cfg
@skip_ad_test
def test_50_Deleting_test_file_and_directory_from_SMB_share():
...
@osx_host_cfg
@skip_ad_test
def test_51_Verifying_that_test_file_directory_successfully_removed():
...
@osx_host_cfg
@skip_ad_test
def test_53_Removing_SMB_mountpoint():
    cmd = 'test -d "%s" && rmdir "%s" || exit 0' % (MOUNTPOINT, MOUNTPOINT)
    results = SSH_TEST(cmd, OSX_USERNAME, OSX_PASSWORD, OSX_HOST)
    assert results['result'] is True, results['output']
...
@skip_ad_test
def test_54_disable_activedirectory():
    global payload, results
    payload = {
        "enable": False
    }
...
@skip_ad_test
def test_55_get_activedirectory_state():
    results = GET('/activedirectory/get_state/')
    assert results.status_code == 200, results.text
    assert results.json() == 'DISABLED', results.text
...
@skip_ad_test
def test_56_get_activedirectory_started_after_disabling_AD():
    results = GET('/activedirectory/started/')
    assert results.status_code == 200, results.text
    assert results.json() is False, results.text
...
@skip_ad_test
def test_57_re_enable_activedirectory():
    global payload, results
    payload = {
        "enable": True
    }
...
@skip_ad_test
def test_58_get_activedirectory_started():
    results = GET('/activedirectory/started/')
    assert results.status_code == 200, results.text
    assert results.json() is True, results.text
...
@skip_ad_test
def test_60_leave_activedirectory():
    global payload, results
    payload = {
        "username": ADUSERNAME,
        "password": ADPASSWORD
...
@skip_ad_test
def test_62_get_activedirectory_started_after_living():
    results = GET('/activedirectory/started/')
    assert results.status_code == 200, results.text
    assert results.json() is False, results.text
...
@skip_ad_test
def test_63_disable_cifs_service_at_boot():
    results = PUT("/service/id/cifs/", {"enable": False})
    assert results.status_code == 200, results.text


@skip_ad_test
def test_64_checking_to_see_if_clif_service_is_enabled_at_boot():
    results = GET("/service?service=cifs")
    assert results.json()[0]["enable"] is False, results.text


@skip_ad_test
def test_65_stoping_clif_service():
    payload = {"service": "cifs", "service-control": {"onetime": True}}
...
@skip_ad_test
def test_66_checking_if_cifs_is_stop():
    results = GET("/service?service=cifs")
    assert results.json()[0]['state'] == "STOPPED", results.text


@skip_ad_test
def test_67_destroying_ad_dataset_for_smb():
    results = DELETE(f"/pool/dataset/id/{dataset_url}/")
    assert results.status_code == 200, results.text


@skip_ad_test
def test_68_configure_setting_domain_hostname_and_dns():
    if ad_dns is True:
