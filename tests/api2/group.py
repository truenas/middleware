...
def test_11_get_group_new_info():
    global groupinfo
    groupinfo = GET('/group?group=newgroup').json()[0]


def test_12_look_group_name():
    assert groupinfo["group"] == "newgroup"


def test_13_look_user_new_uid():
    assert groupinfo["gid"] == new_next_gid


def test_14_look_for_testgroup_is_not_in_freenas_group():
    cmd = 'getent group | grep -q testgroup'
    results = SSH_TEST(cmd, 'root', 'testing', ip)
    assert results['result'] is False, results['output']


def test_15_look_for_newgroup_is_in_freenas_group():
...
# Delete the group
def test_16_delete_group_testgroup_newgroup():
    results = DELETE(f"/group/id/{groupid}/", {"delete_users": True})
    assert results.status_code == 200, results.text


def test_17_look_group_is_delete():
    assert len(GET('/group?group=newuser').json()) == 0


def test_18_look_for_newgroup_is_not_in_freenas_group():
    cmd = 'getent group | grep -q newgroup'
    results = SSH_TEST(cmd, 'root', 'testing', ip)
    assert results['result'] is False, results['output']
