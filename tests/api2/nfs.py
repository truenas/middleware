...
def test_03_starting_nfs_service_at_boot():
    results = PUT("/service/id/nfs", {"enable": True})
    assert results.status_code == 200, results.text


def test_04_checking_to_see_if_nfs_service_is_enabled_at_boot():
    results = GET("/service?service=nfs")
    assert results.json()[0]["enable"] == True, results.text


def test_05_starting_nfs_service():
    payload = {"service": "nfs", "service-control": {"onetime": True}}
    results = POST("/service/start", payload)
    assert results.status_code == 200, results.text


def test_06_checking_to_see_if_nfs_service_is_running():
    results = GET("/service?service=nfs")
...
@bsd_host_cfg
# Now check if we can mount NFS / create / rename / copy / delete / umount
def test_17_creating_nfs_mountpoint():
    results = SSH_TEST('mkdir -p "%s"' % MOUNTPOINT,
                       BSD_USERNAME, BSD_PASSWORD, BSD_HOST)
    assert results['result'] is True, results['output']
...
@mount_test_cfg
@bsd_host_cfg
def test_24_removing_nfs_mountpoint():
    cmd = 'test -d "%s" && rmdir "%s" || exit 0' % (MOUNTPOINT, MOUNTPOINT)
    results = SSH_TEST(cmd, BSD_USERNAME, BSD_PASSWORD, BSD_HOST)
    assert results['result'] is True, results['output']


def test_25_stoping_nfs_service():
    payload = {"service": "nfs", "service-control": {"onetime": True}}
