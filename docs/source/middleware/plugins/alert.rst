...
.. autoclass:: middlewared.alert.base.AlertClassConfig
    :members:
...
a class may narrow the settings catalogue further with `listed_only_when`. A class states both in its
`AlertClassConfig`, documented above; a source states `applies_to` on itself, as a class variable, and
has no `listed_only_when`.
