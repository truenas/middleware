...
.else
BUILD_TIMESTAMP!=date -u '+%Y%m%d%k%M'
.endif
...
.endif
ENV_SETUP=env NANO_LABEL=${NANO_LABEL} VERSION=${VERSION} GIT_LOCATION=${GIT_LOCATION} BUILD_TIMESTAMP=${BUILD_TIMESTAMP} SEQUENCE=${BUILD_TIMESTAMP} NANO_NAME=${TRAIN}-${BUILD_TIMESTAMP}
