...
def GenerateInterfaceGraph(time):
  ...
  for file in filter(filterOctet, os.listdir(rrd_dir)):
    ...
    if not Cache.IsValid(path):
      rrdtool.graph('/tmp/if-%s-%s.png' % (iface, time),
        '--imgformat', 'PNG', '--vertical-label', 'Bits per second',
        '--title', 'Interface Traffic (%s)' % iface, '--lower-limit', '0',
...
def GenerateCpuGraph(time):
  ...
  rrdtool.graph("/tmp/%s-%s.png" % (file, time),
    '--imgformat', 'PNG', '--vertical-label', '%CPU',
    '--title', 'CPU Usage', '--lower-limit', '0',
    '--end', 'now', '--start', 'end-%s' % time,
...
def GenerateMemoryGraph(time):
  ...
  file = "memory"
  rrdtool.graph("/tmp/%s-%s.png" % (file, time),
    '--imgformat', 'PNG', '--vertical-label', 'Bytes',
    '--title', 'Physical memory utilization', '--lower-limit', '0',
    '--end', 'now', '--start', 'end-%s' % time, '-b', '1024',
...
def GenerateSystemLoadGraph(time):
  load = "/var/db/collectd/rrd/localhost/load/load.rrd"
  file = "load"
  rrdtool.graph("/tmp/%s-%s.png" % (file, time),
    '--imgformat', 'PNG', '--vertical-label', 'System Load',
    '--title', 'System Load', '--lower-limit', '0',
    '--end', 'now', '--start', 'end-%s' % time,
...
def GenerateProcessGraph(time):
  ...
  rrdtool.graph("/tmp/%s-%s.png" % (file, time),
    '--imgformat', 'PNG', '--vertical-label', 'Processes',
    '--title', 'Processes', '--lower-limit', '0',
    '--end', 'now', '--start', 'end-%s' % time,
...
def GenerateSwapGraph(time):
  ...
  rrdtool.graph("/tmp/%s-%s.png" % (file, time),
    '--imgformat', 'PNG', '--vertical-label', 'Bytes',
    '--title', 'Swap Utilization', '--lower-limit', '0',
    '--end', 'now', '--start', 'end-%s' % time,
...
def GenerateDiskspaceGraph(time):
  ...
  for file in filter(filterDisk, os.listdir(ds)):
    ...
    if not Cache.IsValid(path):
      rrdtool.graph("/tmp/df-%s-%s.png" % (volname, time),
        '--imgformat', 'PNG', '--vertical-label', 'Bytes',
        '--title', 'Diskspace (%s)' % volname, '--lower-limit', '0',
        '--end', 'now', '--start', 'end-%s' % time,
...
for time in ['1h', '1d', '1w', '1m', '1y']:
  GenerateInterfaceGraph(time)
  GenerateCpuGraph(time)
  GenerateMemoryGraph(time)
  GenerateSystemLoadGraph(time)
  GenerateProcessGraph(time)
  GenerateSwapGraph(time)
  GenerateDiskspaceGraph(time)
