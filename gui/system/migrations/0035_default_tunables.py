...
def get_default_tunables():
    """Get the default arch-specific set of loader values

    :Returns:
        a list of tuples in the following format: (varname, value, comment, )
        or an empty list if the architecture isn't supported.
    """

    arch_width = int(platform.architecture()[0].replace('bit', ''))

    if arch_width == 32:
        kern__ipc__nmbclusters = 25000
        vfs__zfs__arc_max = 512
        vm__kmem_size = 768

    elif arch_width == 64:
        kern__ipc__nmbclusters = 50000
        vfs__zfs__arc_max = 1024
        vm__kmem_size = 1536
    else:
        return []

    # Varname, value, comment (None if you want to try generating the comment
    # from sysctl -dn).
    tunables = [
        (
          'kern.ipc.nmbclusters',
          kern__ipc__nmbclusters,
          None,
        ),
        (
          'vfs.zfs.arc_max',
          '%sM' % (vfs__zfs__arc_max, ),
          None,
        ),
        (
          'vm.kmem_size',
    ...
    for i, tunable_tuple in enumerate(tunables):
        varname, value, comment = tunable_tuple
        if comment is None:
            pipe = os.popen('/sbin/sysctl -dn %s' % (varname, ))
            try:
                comment = pipe.readlines()[0].strip()
            finally:
                pipe.close()
            tunables[i] = (varname, value, comment, )
