...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'SystemDataset.sys_uuid'
        db.add_column(u'system_systemdataset', 'sys_uuid',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=32),
                      keep_default=False)

        if not db.dry_run:
            for obj in orm['system.SystemDataset'].objects.all():
                obj.sys_uuid = uuid.uuid4().hex
                obj.save()


    def backwards(self, orm):
        # Deleting field 'SystemDataset.sys_uuid'
        db.delete_column(u'system_systemdataset', 'sys_uuid')
