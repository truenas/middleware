...
class Migration(SchemaMigration):
    ...
    def backwards(self, orm):
        # Deleting field 'Settings.stg_directoryservice'
        db.delete_column('system_settings', 'stg_directoryservice')

        if not db.dry_run:
            srv = models.services.objects.filter(srv_service='directoryservice')
            srv.delete()
