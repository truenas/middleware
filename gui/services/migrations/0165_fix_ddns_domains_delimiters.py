...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        o = orm.DynamicDNS.objects.all()[0]
        if o.ddns_domain:
            new_ddns_domain = ""
            array = o.ddns_domain.split(',')
            for i in range(0, len(array)):
                element = array[i].strip()
                next_index = i + 1
                new_ddns_domain = new_ddns_domain + element
                if (next_index < len(array)):
                    if array[next_index].strip().isalnum():
                        new_ddns_domain = new_ddns_domain + "#"
                    else:
                        new_ddns_domain = new_ddns_domain + "," 
            o.ddns_domain = new_ddns_domain
            o.save()
