...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'ReplRemote.ssh_cipher'
        db.add_column(u'storage_replremote', 'ssh_cipher',
                      self.gf('django.db.models.fields.CharField')(default='standard', max_length=20),
                      keep_default=False)

        if not db.dry_run:
            orm['storage.ReplRemote'].objects.filter(ssh_fast_cipher=True).update(
                ssh_cipher='fast'
            )

            orm['storage.ReplRemote'].objects.filter(ssh_no_cipher=True).update(
                ssh_cipher='disabled'
            )


    def backwards(self, orm):
        # Deleting field 'ReplRemote.ssh_cipher'
        db.delete_column(u'storage_replremote', 'ssh_cipher')
