...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for e in orm['services.iSCSITargetExtent'].objects.filter(iscsi_target_extent_type='ZVOL'):
            if not e.iscsi_target_extent_path.isdigit():
                continue
            disk = orm.Disk.objects.filter(id=e.iscsi_target_extent_path)
            if disk.exists():
                disk = disk[0]
                e.iscsi_target_extent_path = disk.disk_identifier.split('}',1)[1]
                e.save()
                disk.delete()
            else:
                e.delete()
    ...
    complete_apps = ['services', 'storage']
