...
class Migration(DataMigration):
    ...
    def save_principals(self, orm, keytab):
        if not keytab:
            return False
        ...
        p = Popen(['/usr/sbin/ktutil', '-vk', tmpfile, 'list'],
    ...
    def forwards(self, orm):
        for kt in orm.KerberosKeytab.objects.all():
            self.save_principals(orm, kt)

    def backwards(self, orm):
        for kt in orm.KerberosKeytab.objects.all():
            orm.KerberosPrincipal.objects.filter(
                principal_keytab=kt
            ).delete() 
