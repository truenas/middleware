...
class Migration(DataMigration):
    ...
    def fix_backend(self, backend):
        if backend == "idmap_ad":
            return "ad"
        if backend == "idmap_autorid":
            return "autorid"
        if backend == "idmap_hash":
            return "hash"
        if backend == "idmap_ldap":
            return "ldap"
        if backend == "idmap_nss":
            return "nss"
        if backend == "idmap_rfc2307":
            return "rfc2307"
        if backend == "idmap_rid":
            return "rid"
        if backend == "idmap_tdb":
            return "tdb"
        if backend == "idmap_tdb2":
            return "tdb2"
        return None 
    ...
    def forwards(self, orm):
        try:
            ...
            ad.ad_idmap_backend = self.fix_backend(ad.ad_idmap_backend)
            ad.save()
        ...
        try:
            ...
            ldap.ldap_idmap_backend = self.fix_backend(ldap.ldap_idmap_backend)
            ldap.save()
        ...
        try:
            ...
            nt4.nt4_idmap_backend = self.fix_backend(nt4.nt4_idmap_backend)
            nt4.save()
