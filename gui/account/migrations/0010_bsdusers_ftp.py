...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        if orm.bsdUsers.objects.filter(models.Q(bsdusr_username='ftp')|models.Q(bsdusr_uid=14)).count() == 0:
            usr = orm.bsdUsers()
            usr.bsdusr_uid = 14
            usr.bsdusr_username = 'ftp'
            usr.bsdusr_group = orm.bsdGroups.objects.get(bsdgrp_group='ftp')
            usr.bsdusr_builtin = True
            usr.save()

    def backwards(self, orm):
        "Write your backwards methods here."

        orm.bsdUsers.objects.get(bsdusr_uid=14).delete()
