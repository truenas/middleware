...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for cifs in orm['services.CIFS'].objects.all():
            if not cifs.cifs_srv_homedir_enable:
                continue
            share = orm['sharing.CIFS_Share']()
            share.cifs_home = True
            share.cifs_path = cifs.cifs_srv_homedir
            share.cifs_browsable = cifs.cifs_srv_homedir_browseable_enable
            share.cifs_auxsmbconf = cifs.cifs_srv_homedir_aux
            share.save()
    ...
    complete_apps = ['services', 'sharing']
    symmetrical = True
