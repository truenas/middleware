...
def runTest():
    ...
    # Part 1:  Do an upgrade install using the ISO
    ...
    child2.sendline("\n")
    child2.expect(['Do you wish to perform an upgrade'])
    child2.sendline("U")
    child2.expect(['FreeNAS will migrate this file'], 250)
    child2.sendline("O")
    ...
    child5 = pexpect.spawn("bhyveload -m 2G -d %s %s" % (test_config['disk_img'], test_config['vm_name']))
    child5.logfile = sys.stdout
    child5.expect (['Booting...'])
    child5.expect(pexpect.EOF)
    child6 = pexpect.spawn("bhyve -c 2 -m 2G -AI -H -P -g 0 -s 0:0,hostbridge -s 1:0,lpc -s 2:0,virtio-net,%s -s 31:0,virtio-blk,%s -l com1,stdio %s" % (test_config['tap'], test_config['disk_img'], test_config['vm_name']))
