common_installfs()
{
	...
	STAGE_DIR="${NANO_OBJ}/_.isodir" # Directory ISO is rolled from
...
build_installfs()
{
	...
	if [ "x$STAGE_DIR" = "x" ] ; then
	    echo "build_installfs missing parameter."
	    exit 1
	fi
	...
	mkdir -p ${STAGE_DIR}/dev
	mkdir -p ${STAGE_DIR}/.mount
	...
	tar -c -f - -C ${NANO_OBJ}/_.w --exclude boot/kernel-debug boot | tar -x -f - -C ${STAGE_DIR}
	ln -f $IMGFILE $STAGE_DIR/$NANO_LABEL-$NANO_ARCH_HUMANIZED.img.xz
	...
	mkuzip -o ${STAGE_DIR}/data/base.ufs.uzip ${TEMP_IMGFILE}
