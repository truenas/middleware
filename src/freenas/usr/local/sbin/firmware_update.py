...
if not os.path.exists(SAS2FLASH):
    logger.info("%s not installed, skipping SAS92xx HBA firmware check", SAS2FLASH)
else:
...
if not os.path.exists(SAS3FLASH):
    logger.info("%s not installed, skipping SAS93xx HBA firmware check", SAS3FLASH)
else:
...
if not os.path.exists(STORCLI):
    logger.info("%s not installed, skipping HBA94xx HBA firmware check", STORCLI)
else:
