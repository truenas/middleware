...
def dav_passwd_change(passwd,auth_type):
        ...
        _pipeopen(cmd)
        _pipeopen("chown webdav:webdav /etc/local/apache24/webdavht%s" % (auth_type,))
...
def main():
    ...
    total_shares = WebDAV_Share.objects.count()
    ...
    if dav_protocol in ['https','httphttps']:
        ...
        with open(davssl_config_file,'w') as f2:
            ...
            # different files are being written to I could not at the moment find a
            # more efficient way of doing this. (if you can fix it, please do so)
            for share in webshares:
                temp_path=""" "%s" """ % share.webdav_path
                ...
                # that we do not waste time in changin permisions twice (once while in http block) 
                if (share.webdav_perm and dav_protocol):
                    _pipeopen("chown -R webdav:webdav %s" % share.webdav_path)
