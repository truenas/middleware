...
class LicenseReconcileDelegate:
    ...
    declarable without making any call, because it is what uniqueness checking and coverage
    tests are written against. No two delegates may claim the same group.
    ...
    * `RELOAD` and `RESTART` delegates are rendered by `service.control`, which regenerates the
      ...
      responsible for so that uniqueness checking and coverage tests have something to work
      against -- rather than a list of groups anyone renders from.
    """
    ...
    order: int = 0
    # Prose explaining why this subsystem is license-sensitive. This travels with the code
    # so that the next person to touch it knows what the license actually changes here.
    reason: str = NotImplemented
