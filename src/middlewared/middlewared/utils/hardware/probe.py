...
from truenas_pydmi.errors import DMIError
...
def get_hardware_info() -> HardwareInfo:
    ...
    Not cached: ``cached_dmi()`` and ``detect_platform()`` are each
    ``@cache``d and ``classify`` is pure.
    """
    try:
        chassis = cached_dmi().tn_model
        ha_platform: str = detect_platform()[0]
    except DMIError:
        return classify(chassis=TRUENAS_UNKNOWN, ha_platform="MANUAL")
    return classify(chassis=chassis, ha_platform=ha_platform)
