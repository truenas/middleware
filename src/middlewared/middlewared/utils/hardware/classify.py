...
exercised with plain strings without mocking. ``probe`` owns the impure steps
-- reading DMI and running ``detect``'s platform detection -- and hands both
results here as plain data.
"""
...
def classify_platform(*, chassis: str, ha_platform: str) -> Platform:
    """Classify `chassis` into a ``Platform``.
...
def classify(*, chassis: str, ha_platform: str) -> HardwareInfo:
    platform = classify_platform(chassis=chassis, ha_platform=ha_platform)
    return HardwareInfo(
        platform=platform,
