...
from middlewared.utils.iscsi.constants import (
    ...
    ALUA_GROUP_ID_B,
    ALUA_STATE,
    REL_TGT_ID_FC_OFFSET,
    REL_TGT_ID_NODEB_OFFSET,
)
...
# PR APTPL metadata directory under the kernel's compiled-in db_root
# (target_core_configfs.c: DB_ROOT_DEFAULT = "/var/target"). Every
# PERSISTENT RESERVE OUT REGISTER writes an APTPL file here -- via
# core_scsi3_update_and_write_aptpl() -- regardless of whether APTPL was
# actually requested; if the directory doesn't exist the write fails and
# the REGISTER itself fails with LOGICAL UNIT COMMUNICATION FAILURE.
DB_ROOT_PR_DIR = pathlib.Path("/var/target/pr")
...
def _set_storage_object_attribs(so_dir: pathlib.Path, alua_standby: bool = False):
    ...
    # On ACTIVE, a per-SO failure here means that SO's backend never opened
    # (kernel marks it NOT READY).  We log and continue so the remaining SOs
    # still get configured -- aborting the whole render would leave healthy
    # devices half-configured and complicate idempotent recovery.
    if not alua_standby:
        ...
        if p.exists():
            try:
                ...
                logger.warning(
                    "lio: emulate_tpu write failed on %s (errno=%s); backend likely not open, device will be NOT READY",
                    so_dir.name,
                    e.errno,
                )
...
def _build_iscsi_desired(render_ctx: dict) -> dict:
    ...
    # alua_ha drives phantom TPG generation; see REL_TGT_ID_NODEB_OFFSET in
    # middlewared.utils.iscsi.constants for the full tag assignment table.
    alua_ha = alua and render_ctx.get("failover.status", "SINGLE") != "SINGLE"
    ...
    for target in render_ctx["iscsi.target.query"]:
        ...
        for group in target.get("groups", []):
            ...
            tag = target["rel_tgt_id"] + REL_TGT_ID_NODEB_OFFSET if (alua_ha and node == "B") else target["rel_tgt_id"]
        ...
        # In HA ALUA mode add a phantom TPG for each real TPG so that RTPG
        # reports both controller groups from every connected target port.
        # Phantom TPGs have no portals (invisible to discovery) but carry LUNs
        # assigned to the remote controller group so initiators can see both
        # ALUA groups in a single RTPG response.
        if alua_ha and tpgs:
            remote_group = ALUA_GROUP_B if node == "A" else ALUA_GROUP_A
            for real_tag in list(tpgs.keys()):
                if node == "A":
                    phantom_tag = real_tag + REL_TGT_ID_NODEB_OFFSET
                else:
                    phantom_tag = real_tag - REL_TGT_ID_NODEB_OFFSET
                tpgs[phantom_tag] = {
                    "tag": phantom_tag,
                    ...
                    "chap_mutual_user": "",
                    "chap_mutual_password": "",
                    "luns": tpgs[real_tag]["luns"],
                    "acls": {},
                    "alua_group": remote_group,
                }
...
def _update_iscsi_target(target_dir: pathlib.Path, target_cfg: dict, render_ctx: dict):
    ...
    # Release before claim: a tag change (e.g. ALUA being toggled) can leave a
    # to-be-deleted OR to-be-updated TPG holding the same network portal that
    # a to-be-created OR to-be-updated TPG on this same target needs next.
    # Deleting stale TPGs and updating persisting ones (which may drop a
    # portal they no longer need, e.g. downgrading from real to phantom TPG)
    # must both happen before creating new TPGs (which may claim that same
    # portal), or the claim races the release and the kernel rejects it.
...
def _create_iscsi_tpg(target_dir: pathlib.Path, tpg_cfg: dict, render_ctx: dict):
    ...
    # tpg_rtpi is auto-assigned sequentially by xa_alloc unless we set it
    # explicitly via the rtpi attribute before enable.  Write it now so that
    # the relative target port identifier in RTPG matches our tag formula
    # (portal.tag on Node A, portal.tag + REL_TGT_ID_NODEB_OFFSET on Node B).
    rtpi_path = tpg_dir / "rtpi"
    if rtpi_path.exists():
        _write(rtpi_path, str(tag))
...
def _reconcile_portals(tpg_dir: pathlib.Path, tpg_cfg: dict):
    ...
    for key in live - set(desired):
        ...
        try:
            ...
            logger.warning("Failed to remove network portal %s: %s", portal_dir, e)
...
def _reconcile_luns(tpg_dir: pathlib.Path, tpg_cfg: dict, render_ctx: dict):
    ...
    # Phantom TPGs supply an explicit 'alua_group' (the remote controller group);
    # real TPGs use the local controller group derived from failover.node.
    ha_mode = render_ctx.get("failover.status", "SINGLE") != "SINGLE" and render_ctx.get("iscsi.global.config", {}).get(
        "alua"
    )
    if "alua_group" in tpg_cfg:
        alua_target_group = tpg_cfg["alua_group"]
        ha_mode = True  # phantom TPG -- still needs alua_tg_pt_gp writes
    elif ha_mode:
        node = render_ctx.get("failover.node", "A")
        alua_target_group = ALUA_GROUP_A if node == "A" else ALUA_GROUP_B
    else:
        ...
        if lun_id not in live_lun_dirs:
            ...
            if ha_mode:
                ...
                if _wait_for(alua_attr, retries=5):
                    _write(alua_attr, alua_target_group)
...
def _fc_rel_tgt_id(target: dict, fcport: dict) -> int:
    """Return the Node A rel_tgt_id for a target on a specific FC port.

    Formula: target.rel_tgt_id + REL_TGT_ID_FC_OFFSET + (hba_index * 1000)
    where hba_index is the numeric suffix of the HBA name (fc0 -> 0, fc1 -> 1).
    Does not include the Node B offset (+REL_TGT_ID_NODEB_OFFSET).
    """
    try:
...
def _build_fc_desired(render_ctx: dict) -> dict:
    ...
    Returns {wwpn_colon_hex: {
        ...
        'tpgs': {tag: {
            ...
            'acls': {},
            'generate_node_acls': bool,
            # 'alua_group': str  -- phantom TPGs only
        }},
    }}
    """
    ...
    alua = render_ctx.get("iscsi.global.config", {}).get("alua", False)
    alua_standby = alua and render_ctx.get("failover.status") == "BACKUP"
    # alua_ha drives phantom TPG generation; see REL_TGT_ID_NODEB_OFFSET in
    # middlewared.utils.iscsi.constants for the full tag assignment table.
    alua_ha = alua and render_ctx.get("failover.status", "SINGLE") != "SINGLE"
    ...
    for fcport in render_ctx["fcport.query"]:
        ...
        base_tag = _fc_rel_tgt_id(target, fcport)
        real_tag = base_tag + REL_TGT_ID_NODEB_OFFSET if (alua_ha and node == "B") else base_tag
        ...
        if alua_ha:
            remote_group = ALUA_GROUP_B if node == "A" else ALUA_GROUP_A
            phantom_tag = real_tag + REL_TGT_ID_NODEB_OFFSET if node == "A" else real_tag - REL_TGT_ID_NODEB_OFFSET
            tpgs[phantom_tag] = {
                "tag": phantom_tag,
                "luns": target_luns[target["id"]],
                "acls": {},
        ...
        desired[wwpn] = {"wwpn": wwpn, "tpgs": tpgs}
...
def _configure_fc_target(target_dir: pathlib.Path, fc_cfg: dict, render_ctx: dict):
    """Create all FC TPGs (real + phantom in HA ALUA mode) with LUNs."""
    for tpg_cfg in fc_cfg["tpgs"].values():
        tag = tpg_cfg["tag"]
        tpg_dir = target_dir / f"tpgt_{tag}"
        tpg_dir.mkdir()
        rtpi_path = tpg_dir / "rtpi"
...
def _update_fc_target(target_dir: pathlib.Path, fc_cfg: dict, render_ctx: dict):
    """Reconcile FC TPGs -- add, update, and remove as needed."""
    live_tpgs = {int(d.name.split("_")[1]) for d in target_dir.iterdir() if d.is_dir() and d.name.startswith("tpgt_")}
    desired_tags = set(fc_cfg["tpgs"].keys())
    ...
    for tag in live_tpgs - desired_tags:
        _delete_iscsi_tpg(target_dir / f"tpgt_{tag}")


def _delete_fc_target(target_dir: pathlib.Path):
    """Remove a FC target and all its TPGs."""
    for d in list(target_dir.iterdir()):
        if d.is_dir() and d.name.startswith("tpgt_"):
            _delete_iscsi_tpg(d)
    try:
...
def _load_modules(render_ctx: dict):
    ...
    """
    # Must exist before target_core_mod is loaded / any target comes up, or
    # the first PERSISTENT RESERVE OUT REGISTER against any LUN fails --
    # see DB_ROOT_PR_DIR.
    DB_ROOT_PR_DIR.mkdir(parents=True, exist_ok=True)
    ...
    if alua_enabled and not MOD_LIO_HA.exists():
        ...
        # that blocks rmmod (see teardown_lio_config()).  Clear it first so
        # module_put() is called before we try to unload.
        ...
        if _fwd_active.exists():
            try:
                ...
                pass
        result = subprocess.run(["modprobe", "-r", "lio_ha"], capture_output=True)
        if result.returncode != 0:
            logger.warning(
                "Failed to unload lio_ha module: %s",
...
def _alua_groups(render_ctx: dict):
    ...
    state according to the state table:

      MASTER  + synced:        local=ACTIVE_OPTIMIZED   remote=NONOPTIMIZED
      MASTER  + connected:     local=ACTIVE_OPTIMIZED   remote=TRANSITIONING
      MASTER  + disconnected:  local=ACTIVE_OPTIMIZED   remote=UNAVAILABLE
      BACKUP  + synced:        local=NONOPTIMIZED       remote=ACTIVE_OPTIMIZED
      BACKUP  + connected:     local=TRANSITIONING      remote=ACTIVE_OPTIMIZED
      BACKUP  + disconnected:  local=TRANSITIONING      remote=ACTIVE_OPTIMIZED
    ...
    ha_state is "synced" on both nodes once LUN_SYNC_DONE is sent (ACTIVE) or
    received (STANDBY).  UNAVAILABLE for the remote group on a disconnected
    MASTER signals to initiators that Node B paths are genuinely unavailable;
    LIO generates ASC 0x2A/0x06 (ASYMMETRIC ACCESS STATE CHANGED) UAs on each
    state change so initiators do not need to poll.
    ...
    if ha_mode:
        ...
        if is_master:
            ...
            if synced:
                remote_state = ALUA_STATE.NONOPTIMIZED
            elif ha_state == "disconnected":
                remote_state = ALUA_STATE.UNAVAILABLE
            else:
                remote_state = ALUA_STATE.TRANSITIONING
        else:
    ...
    try:
        ...
        # Exit: remove controller groups if ALUA is now disabled.  LUN
        # alua_tg_pt_gp attributes have already been reassigned to
        # default_tg_pt_gp by _reconcile_luns, so the kernel will accept the
        # rmdir.  Runs in `finally` so a later context manager in the same
        # `with` chain (e.g. _iscsi_targets) raising doesn't skip this cleanup.
        if not alua_enabled:
            for backstore in (IBLOCK_DIR, FILEIO_DIR):
                if not backstore.exists():
                    continue
                    ...
                    for group_name in (ALUA_GROUP_A, ALUA_GROUP_B):
                        ...
                        if group_dir.exists():
                            try:
                                ...
                                logger.warning("Failed to remove ALUA group %s: %s", group_dir, e)
