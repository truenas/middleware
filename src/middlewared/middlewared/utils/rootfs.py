...
class ReadonlyRootfsManager:
    ...
    def _handle_usr(self, readonly):
        ...
        if readonly:
            os.chmod(os.path.join(self.root, "usr/bin/dpkg"), 0o644)
            with contextlib.suppress(FileNotFoundError):
                os.rename(os.path.join(self.root, "usr/local/bin/dpkg.bak"),
                          os.path.join(self.root, "usr/local/bin/dpkg"))
        else:
            os.chmod(os.path.join(self.root, "usr/bin/dpkg"), 0o755)
            with contextlib.suppress(FileNotFoundError):
                os.rename(os.path.join(self.root, "usr/local/bin/dpkg"),
                          os.path.join(self.root, "usr/local/bin/dpkg.bak"))
