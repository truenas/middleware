...
def build_nsupdate_plan(ops: list[dict]) -> NSUpdatePlan:
    ...
    for entry in ops:
        ...
        forward.append(nsupdate_directive(
            entry['command'], entry['name'], entry['ttl'], entry['type'], addr.compressed
        ))
        if entry['do_ptr']:
            reverse.append((
...
def run_nsupdate_plan(plan: NSUpdatePlan, send: Callable[[list[str]], subprocess.CompletedProcess]) -> NSUpdateResult:
