...
def _numa_to_pkg() -> dict[int, int]:
    ...
    if not os.path.isdir(node_root):
        return out
    try:
        entries = list(os.scandir(node_root))
    except OSError:
        return out
...
def _resolve_chip_package(
    ...
    entries on dual-EPYC, etc.). Without an explicit chip-to-socket
    mapping the temperature pipeline has no way to know that
    ``coretemp-isa-0001``'s ``"Core 0"`` is a different physical core
    from ``coretemp-isa-0000``'s ``"Core 0"``, and readings get
    silently mis-attributed. That's exactly the kind of bug this
...
def _discover_cpu_chips() -> tuple[_Chip, ...]:
    ...
    if not os.path.isdir(hwmon_root):
        return ()
    try:
        entries = list(os.scandir(hwmon_root))
    except OSError:
        return ()
