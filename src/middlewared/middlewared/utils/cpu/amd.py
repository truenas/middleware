...
def _amd_assignments(
    ...
    # Both sides are driven by the same firmware slot numbering --
    # k10temp_get_ccd_support() in drivers/hwmon/k10temp.c walks absolute SMN
    # slots and exposes Tccd<i+1> only when ZEN_CCD_TEMP_VALID is set, and
    # die_id comes from CPUID 0x8000001E node_id in arch/x86/kernel/cpu/
    # topology_amd.c -- but neither file enforces the correspondence.
    if not prefer_tdie:
