...
__all__ = ['PWENC_CHECK', 'PWENC_PADDING', 'PWENC_FILE_SECRET', 'PWENC_FILE_SECRET_MODE', 'pwenc_rename',
           'pwenc_encrypt', 'pwenc_decrypt', 'pwenc_generate_secret', 'pwenc_secret_matches']
...
def pwenc_secret_matches(secret_path: str, pwenc_check: str) -> bool:
    ...
    The secret is opened directly rather than through the cached global context so that a secret
    belonging to another system can be checked without disturbing ours.
    """
    try:
        ...
        # aes-256-ctr is unauthenticated, so a wrong secret yields garbage rather than raising.
        return False
