...
if typing.TYPE_CHECKING:
    ...
    from .auth import SessionManagerCredentials, TokenSessionManagerCredentials
    from .main import Middleware
    from .pipe import Pipe


def parse_credentials(request: Request) -> dict | None:
...
async def authenticate(
    app: App, middleware: Middleware, request: Request, credentials: dict, method: HttpVerb, resource: str
) -> 'TokenSessionManagerCredentials | LoginPasswordSessionManagerCredentials | ApiKeySessionManagerCredentials':
    if credentials['credentials'] == 'TOKEN':
        ...
        user_authenticator = UserPamAuthenticator()
        app.authentication_context.pam_hdl = user_authenticator
        ...
        key_authenticator = ApiKeyPamAuthenticator()
        app.authentication_context.pam_hdl = key_authenticator
...
def create_application_impl(
    request: Request, credentials: SessionManagerCredentials | None = None
) -> 'Application':
...
def copy_multipart_to_pipe(loop: AbstractEventLoop, filepart: BodyPartReader, pipe: Pipe) -> None:
    try:
        try:
            while True:
