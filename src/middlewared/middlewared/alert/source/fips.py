...
class FIPSProviderAlertSource(AlertSource):
    ...
    async def check(self) -> list[Alert[Any]] | Alert[Any] | None:
        fips_configured = (await self.middleware.call2(self.middleware.services.system.security.config)).enable_fips
        configuration = "enabled" if fips_configured else "disabled"

        try:
            fips_enabled = await self.middleware.call2(self.middleware.services.system.security.info.fips_enabled)
        except Exception:
