...
@dataclass(kw_only=True)
class TierSpecialVdevCriticalAlert(AlertClass):
    config = AlertClassConfig(
...
@dataclass(kw_only=True)
class TierSpecialVdevWarningAlert(AlertClass):
    config = AlertClassConfig(
        category=AlertCategory.STORAGE,
...
class _JobAlertsCache:
    ...
    def get(self, error_jobs: list[Any], complete_jobs: list[Any]) -> list[Alert[Any]] | None:
        """Return the alerts cached for this job set, or ``None`` on miss."""
        if self._key != self._key_for(error_jobs, complete_jobs):
            return None
        return list(self._alerts)

    def store(self, error_jobs: list[Any], complete_jobs: list[Any], alerts: list[Alert[Any]]) -> None:
...
class TierJobAlertSource(ThreadedAlertSource):
    ...
    def _check_jobs(self) -> list[Alert[Any]]:
        error_jobs = []
        complete_jobs = []
    ...
    def _check_special_vdev_usage(self, warning_pct: int, critical_pct: int) -> list[Alert[Any]]:
        ...
        for pool in pools:
            ...
            if pct > critical_pct:
                alerts.append(Alert(TierSpecialVdevCriticalAlert(
                    pool_name=pool_name,
                    threshold=critical_pct,
                )))
            elif pct > warning_pct:
                alerts.append(Alert(TierSpecialVdevWarningAlert(
                    pool_name=pool_name,
