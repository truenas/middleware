...
The declaration-reading helpers live here rather than beside ``applies``: ``AlertClass`` and
``AlertSource`` are only reachable from ``middlewared.alert.base``, which reaches back into this
package through ``middlewared.main``, so the bottom layer must stay unaware of them.
"""
...
from .engine import applies, rule_name
...
def applies_for_listing(declaration: type[AlertClass], facts: EntitlementFacts) -> bool:
    ...
    """
    return applies(declaration.config.applies_to, facts) and applies(declaration.config.listed_only_when, facts)


def class_rule_name(declaration: type[AlertClass]) -> str:
    ...
    """
    return rule_name(declaration.config.applies_to)


def source_rule_name(declaration: type[AlertSource]) -> str:
    """The vocabulary name `declaration` gated itself on, for diagnostics.
...
class Applicability:
    ...
    def class_applies(self, klass: type[AlertClass]) -> bool:
        ...
        try:
            ...
            self._class_memo[klass] = result = applies(klass.config.applies_to, self._facts)
            return result
