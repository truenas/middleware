...
def ds_migrate_ldap(conn, ldap):
    ...
    conn.execute(
        ...
        usershell=ldap['ldap_user_shell'], shadowobj=ldap['ldap_shadow_object_class'],
        shadowchg=ldap['ldap_shadow_last_change'], shadowmin=ldap['ldap_shadow_min'], shadowmax=ldap['ldap_shadow_max'],
...
def migrate_idmap_domain(dom, netbios_domain_name) -> dict | None:
    ...
    match out['idmap_backend']:
        ...
        case 'LDAP':
            ...
            out.udpate(orig_opts)
            out['ldap_url'] = ldap_url
        case 'RFC2307':
            if orig_opts.get('ldap_server', '') != 'STANDALONE':
                # The AD variant althought technically existing was never really used by
