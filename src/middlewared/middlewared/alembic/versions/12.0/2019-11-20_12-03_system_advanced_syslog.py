...
def upgrade():
    ...
    op.execute(text("UPDATE system_advanced SET adv_sysloglevel = (SELECT stg_sysloglevel FROM system_settings)"))
    op.execute(text("UPDATE system_advanced SET adv_syslogserver = (SELECT stg_syslogserver FROM system_settings)"))
    op.execute(text("UPDATE system_advanced SET adv_syslog_transport = 'UDP'"))
