...
from .utils import incus_call, incus_call_sync, Status


RE_IMPORT_PERCENTAGE = re.compile(r'(\d+)?%')
...
class VirtVolumeService(CRUDService):
    ...
    async def query(self, filters, options):
        ...
        for storage_device in storage_devices['metadata']:
            entries.append({
                ...
                'content_type': storage_device['content_type'],
    ...
    @job(lock=lambda args: f'virt_volume_import_iso_{args[0]}')
    def import_iso(self, job, data):
        ...
        job.set_progress(95, 'ISO imported as incus volume')
        return self.get_instance__sync(data['name'])
