...
class VirtBackupService(Service):
    ...
    def apply(self, job, data):
        ...
        if os.path.isdir(data['backup_location']):
            found_root = False
            for filename in os.listdir(data['backup_location']):
                if filename == 'root' and os.path.isfile(os.path.join(data['backup_location'], filename)):
                    found_root = True
                elif filename.startswith('volume_') and os.path.isfile(os.path.join(data['backup_location'], filename)):
                    to_restore.append(os.path.join(data['backup_location'], filename))
            ...
            if found_root is False:
                ...
                to_restore.append(os.path.join(data['backup_location'], 'root'))

        verrors.check()

        job.set_progress(10, 'Completed initial validation')
        if to_restore:
            restore_items_len = len(to_restore)
            ...
            self.middleware.call_sync(
                'virt.backup.import_impl', job, data | {'backup_location': to_restore[0]}, target_pool
            )
