...
def ensure_initialized(context: ServiceContext, entry: CloudBackupEntry, credentials: CredentialsEntry) -> None:
    ...
    attrs = entry.attributes.model_dump(by_alias=True)
    cred = credentials.id
    if "bucket" in attrs:
        existing_buckets = [b["Name"] for b in context.call_sync2(context.s.cloudsync.list_buckets, cred)]
        if attrs["bucket"] not in existing_buckets:
            context.call_sync2(context.s.cloudsync.create_bucket, cred, attrs["bucket"])
