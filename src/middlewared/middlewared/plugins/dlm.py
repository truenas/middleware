...
class DistributedLockManagerService(Service):
    ...
    async def join_lockspace(self, lockspace_name):
        ...
        try:
            ...
            await asyncio.gather(*[
                self.join_kernel_lockspace(nodeid, lockspace_name, self.nodeID, list(nodeIDs))
                for nodeid in nodeIDs
            ])
        except Exception:
    ...
    async def leave_lockspace(self, lockspace_name):
        ...
        try:
            ...
            await asyncio.gather(*[
                self.leave_kernel_lockspace(nodeid, lockspace_name, self.nodeID)
                for nodeid in nodeIDs
            ])
    ...
    async def remove_node(self, nodeid):
        ...
        if node:
            ...
            for lockspace_name in await self.middleware.call('dlm.kernel.node_lockspaces', nodeid):
                ...
                await asyncio.gather(*[
                    self.leave_kernel_lockspace(node_id, lockspace_name, nodeid)
                    for node_id in nodeIDs
                ])
                await asyncio.gather(*[
                    self.start_kernel_lockspace(node_id, lockspace_name) for node_id in nodeIDs
                ])
    ...
    async def reset_active(self, job):
        ...
        # HA-target logout deliberately handled by standby_after_start, not here.
