...
from middlewared.api.current import (WebShareEntry, WebShareUpdateArgs,
...
class WebShareModel(sa.Model):
    ...
    srv_home_directory_template = sa.Column(sa.String(255), default='{{.Username}}')
...
class WebShareService(SystemServiceService):
    ...
    async def do_update(self, data):
        ...
        - `is_home_base`: Whether this share is the base for user home directories

        Only one share can have `is_home_base` set to true. Home base shares are
        not displayed as regular shares but are used as the base path for dynamic
        home directory creation.

        `home_directory_template` defines the template for home directory names,
        supporting {{.Username}}, {{.UID}}, and {{.GID}} placeholders.

        `home_directory_perms` sets the permissions for newly created home directories.
        ...
        # Start/restart truesearch service if needed, passing old config to detect changes
    ...
    async def _validate(self, data):
        ...
        if data.get('shares') is not None:
            ...
            share_names = [share['name'] for share in data['shares'] if 'name' in share]
            ...
            share_paths = [share['path'] for share in data['shares'] if 'path' in share]
        ...
        # Validate home directory template
        if data.get('home_directory_template') is not None:
            template = data['home_directory_template']
            if not template:
                verrors.add(
                    'webshare_update.home_directory_template',
                    'Home directory template cannot be empty'
                )
            else:
                ...
                has_placeholder = any(placeholder in template for placeholder in valid_placeholders)
                if not has_placeholder:
                    verrors.add(
                        'webshare_update.home_directory_template',
                        'Template must contain at least one placeholder: {{.Username}}, {{.UID}}, or {{.GID}}'
                    )

        # Validate home directory permissions
        if data.get('home_directory_perms') is not None:
            perms = data['home_directory_perms']
            if not perms:
                verrors.add(
                    'webshare_update.home_directory_perms',
                    'Home directory permissions cannot be empty'
                )
            else:
                ...
                try:
                    ...
                    verrors.add(
                        'webshare_update.home_directory_perms',
                        'Invalid permissions format. Use octal notation (e.g., "0755" or "755")'
                    )
    ...
    async def _generate_config_files(self):
        ...
        if config['search_enabled'] or has_search_indexed_shares:
            self.logger.debug(f'Updating truesearch config - global_search_enabled={config["search_enabled"]}, '
                              f'has_search_indexed_shares={has_search_indexed_shares}')
            await self._update_truesearch_config(config, search_index_path)
        else:
            self.logger.debug('Skipping truesearch config update - no search-indexed shares')
    ...
    async def _update_truesearch_config(self, config, search_index_path):
        ...
        self.logger.debug(f'WebShare config has {len(config.get("shares", []))} shares')

        for share in config['shares']:
            self.logger.debug(f'Processing share: {share.get("name")} - '
                              f'is_home_base={share.get("is_home_base", False)}, '
                              f'search_indexed={share.get("search_indexed", True)}, '
                              f'path={share.get("path")}')
            # Include all shares (regular AND home base) that have search_indexed enabled
            if share.get('search_indexed', True):
                path = share.get('path')
                if path and path not in webshare_dirs:
                    webshare_dirs.append(path)
                    self.logger.debug(f'Added share path to webshare directories: {path}')
        ...
        non_webshare_dirs = [d for d in existing_dirs if not d.startswith('/mnt/')]
        ...
        self.logger.debug(f'Updated truesearch directories: {search_config["directories"]}')
    ...
    async def _manage_truesearch_after_config_update(self, config):
        ...
        # Only manage service if global search is enabled OR we have search-indexed shares
        should_run_truesearch = config.get('search_enabled', False) or has_search_indexed_shares

        if not should_run_truesearch:
            self.logger.debug('No search-indexed shares, skipping truesearch service management')
            return
        ...
        try:
            ...
            self.logger.debug(f'Could not check truesearch service status: {e}')
            truesearch_running = False

        if truesearch_running:
            # Service is running - restart it to pick up config changes
            try:
                self.logger.info('Restarting truesearch service to apply configuration changes')
                await self.middleware.call('service.restart', 'truesearch')
            except Exception as e:
                self.logger.warning(f'Failed to restart truesearch service: {e}')
        else:
            # Service is not running - start it
            try:
                self.logger.info('Starting truesearch service for search-indexed shares')
                await self.middleware.call('service.start', 'truesearch')
            except Exception as e:
                self.logger.warning(f'Failed to start truesearch service: {e}')
    ...
    async def _auto_configure_pools_if_needed(self, skip_reload=False):
        ...
        Args:
            skip_reload: If True, update database directly without triggering service reload
        """
        config = await self.config()
        ...
        if update_data:
            if skip_reload:
                # Update database directly without triggering service reload
                # This is used during service startup to avoid circular dependency
    ...
    async def _manage_truesearch_service(self, old_config=None):
        ...
        Args:
            old_config: Previous configuration to detect changes. If None, skip change detection.
        """
        config = await self.config()
        ...
        # TrueSearch should run if search is enabled and there are search-indexed shares
        ...
        if should_run_truesearch and not truesearch_running:
            try:
                self.logger.info(
                    'Starting truesearch service as webshare has search-enabled shares'
                )
                await self.middleware.call('service.start', 'truesearch')
            except Exception as e:
                self.logger.warning(f'Failed to start truesearch service: {e}')
            ...
            self.logger.info('WebShare no longer requires truesearch service')
        elif should_run_truesearch and truesearch_running and old_config is not None:
            ...
            if set(old_dirs) != set(new_dirs):
                try:
                    self.logger.info(
                        'Restarting truesearch service due to directory changes'
                    )
                    await self.middleware.call('service.restart', 'truesearch')
                except Exception as e:
                    self.logger.warning(f'Failed to restart truesearch service: {e}')
    ...
    def _get_search_directories(self, config):
        ...
        # Add all share paths (regular AND home base) that have search_indexed enabled
        for share in config.get('shares', []):
            if share.get('search_indexed', True):
                path = share.get('path')
