...
class DiskService(Service):
    ...
    def format(self, disk, swapgb, sync=True):
        ...
        for command in commands:
            # this is HORRID....this means we subprocess out 2 (or 4) * N (N being number of disks)
            # which means we kill ourselves with lock contention on a large system (100's of drives)
            # does pyparted module work on freeBSD??
            cp = subprocess.run(
