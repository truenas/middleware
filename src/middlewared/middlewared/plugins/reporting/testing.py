...
class ReportingService(Service):
    ...
    def old(self):
        ...
        # Virtual memory use
        data['memory'] = self.middleware.call_sync('reporting.get_memory_info', data['zfs']['arc_size'])
        ...
        if cp_time and cp_times and cp_times_last:
            ...
            for i in range(cp_nums):
                data['cpu'][i] = self.middleware.call_sync('reporting.get_cpu_usages', cp_diff[i * num_times:i * num_times + num_times])

            cp_diff = list(map(lambda x: x[0] - x[1], zip(cp_time, cp_time_last)))
            data['cpu']['average'] = self.middleware.call_sync('reporting.get_cpu_usages', cp_diff)
    ...
    def news(self):
        ...
        data = {
            ...
            'cpu': get_cpu_stats(netdata_metrics, self.middleware.call_sync('system.info')['cores']),
            'disks': get_disk_stats(netdata_metrics, list(self.middleware.call_sync('device.get_disks'))),
            'interfaces': get_interface_stats(
                netdata_metrics, interval, [i['name'] for i in self.middleware.call_sync('interface.query')]
            ),
        }
    ...
    @private
    def get_cpu_usages(self, cp_diff):
