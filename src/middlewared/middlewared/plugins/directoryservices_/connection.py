...
class DomainConnection(
    ...
    def remove_privileges(self, domain: str) -> None:
        ...
        if priv[0]['local_groups']:
            self.logger.warning('%s: unable to remove RBAC privilege associated with domain '
                                'due to presence of local accounts.', domain)
            return
    ...
    def leave_domain(self, job: Job) -> None:
        ...
        if self._test_is_joined(ds_type, domain):
            ...
            raise CallError(
                f'{domain}: domain join is not healthy. Manual cleanup of machine account on '
                'remote domain controller for domain may be required.'
            )
