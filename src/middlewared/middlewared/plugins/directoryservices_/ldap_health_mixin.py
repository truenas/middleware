...
class LDAPHealthMixin:
    ...
    def _ldap_get_dn(self, dn=None, scope_base=True):
        ...
        ldap_config = self.middleware.call_sync('directoryservices.dsconfig_to_ldap_client_config', data)
    ...
    def _ldap_get_root_dse(self, data: dict) -> dict:
        ...
        """
        ldap_config = self.middleware.call_sync('directoryservices.dsconfig_to_ldap_client_config', data)
        return LdapClient.search(ldap_config, '', ldap.scope, '(objectclass=*)')
