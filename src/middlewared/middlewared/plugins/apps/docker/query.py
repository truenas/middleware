...
def list_resources_by_project(project_name: str | None = None) -> dict[str, dict[str, list]]:
    ...
    containers = client.containers.list(all=True, filters=label_filter, sparse=True)
    ...
    for resource_type, resource in chain(
        ...
        projects[resource.attrs['Labels'][PROJECT_KEY]][resource_type].append(resource.attrs)
