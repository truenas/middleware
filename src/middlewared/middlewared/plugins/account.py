...
class UserService(CRUDService):
    ...
    async def do_delete(self, pk, options=None):
        ...
        if await self.middleware.call('notifier.common', 'system', 'domaincontroller_enabled'):
            await self.middleware.call('notifier.samba4', 'user_delete', {'method_args': user['username']})
    ...
    async def __update_sshpubkey(self, user, group):
        ...
        if pubkey is None:
            ...
            if pubkey != oldpubkey:
                ...
                if not os.path.isdir(sshpath):
                    raise CallError(f'{sshpath} must be a valid directory.')
...
class GroupService(CRUDService):
    ...
    async def do_delete(self, pk, options=None):
        ...
        if await self.middleware.call('notifier.common', 'system', 'domaincontroller_enabled'):
            await self.middleware.call('notifier.samba4', 'group_delete', {'method_args': group['group']})
