...
from middlewared.plugins.idmap_.utils import WBClient, IDType
...
class IdmapDomainService(TDBWrapCRUDService):
    ...
    def _pyuidgid_to_dict(self, entry):
        if entry.id_type == wbclient.WBC_ID_TYPE_UID:
            idtype = 'USER'
        elif entry.id_type == wbclient.WBC_ID_TYPE_GID:
            idtype = 'GROUP'
        else:
            idtype = 'BOTH'
        return {
    ...
    def sid_to_name(self, sid):
        ctx = WBClient()
        entry = ctx.sid_to_uidgid_entry(sid)
        return {
            'name': f'{entry.domain}{ctx.separator}{entry.name}',
    ...
    def sid_to_unixid(self, sid_str):
        ...
        return _pyuidgid_to_dict(entry)
    ...
    def unixid_to_sid(self, data):
        ...
        return _pyuidgid_to_dict(entry)['sid']
