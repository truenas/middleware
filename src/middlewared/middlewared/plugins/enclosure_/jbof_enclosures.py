...
def fake_jbof_enclosure(model, num_of_slots, mapped, ui_info):
    ...
    dmi = f'{model.lower()}_jbof_enclosure'
    fake_enclosure = {
        'id': dmi,
        'dmi': dmi,
        'model': model,
...
def map_es24n(model, rclient, uri):
    ...
    return fake_jbof_enclosure(model, num_of_slots, mapped, ui_info)
...
def get_redfish_clients(jbofs):
    ...
    for jbof in jbofs:
        try:
            rclient = RedfishClient(jbof['mgmt_ip1'])
