...
class BootService(Service):
    ...
    async def install_loader(self, dev):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            await self.middleware.run_in_thread(os.makedirs, efi_dir)
            await run('mount', '-t', 'vfat', f'/dev/{partition}', efi_dir)
            grub_cmd = [
                'grub-install', '--target=x86_64-efi', f'--efi-directory={efi_dir}',
                '--bootloader-id=debian', '--recheck', '--no-floppy',
            ]
            if not await self.middleware.run_in_thread(os.path.exists, '/sys/firmware/efi'):
                grub_cmd.append('--no-nvram')
            await run(*grub_cmd)
            mounted_efi_dir = os.path.join(efi_dir, 'EFI')
            await self.middleware.run_in_thread(os.makedirs, os.path.join(mounted_efi_dir, 'boot'), exist_ok=True)
            shutil.copy(
