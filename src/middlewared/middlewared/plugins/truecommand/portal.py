...
from .state import dismiss_alerts, event_config, set_status
from .wireguard import start_truecommand_service, stop_truecommand_service
...
async def post_call(
    context: ServiceContext, options: dict[str, Any] | None = None, payload: dict[str, Any] | None = None,
) -> dict[str, Any]:
    if not await context.middleware.call('network.general.can_perform_activity', 'truecommand'):
        return {'error': 'Network activity denied for TrueCommand service'}

    options = options or {}
    timeout = options.get('timeout', 15)
    response: dict[str, Any] = {'error': None, 'response': {}}
        ...
        async with asyncio.timeout(timeout):
            async with aiohttp.ClientSession(
                ...
                req = await session.post(
                    PORTAL_URI, data=data, headers={'Content-type': 'application/json'},
                )
...
async def poll_api_for_status(context: ServiceContext, job: Job) -> None:
    ...
    while config['enabled']:
        try:
            status = await poll_once(context, config)
        ...
        if status['state'] == PortalResponseState.ACTIVE:
            ...
            if status.get('tc_state') == 'running':
                await dismiss_alerts(context)
                await start_truecommand_service(context)
            else:
                await dismiss_alerts(context, True)
                await context.call2(context.s.alert.oneshot_create, TruecommandContainerHealthAlert())

                asyncio.get_event_loop().call_later(
                    POLLING_GAP_MINUTES * 60,
                    lambda: context.create_task(start_truecommand_service(context)),
                )
            ...
            await dismiss_alerts(context, True)
            await context.call2(
                context.s.alert.oneshot_create, TruecommandConnectionDisabledAlert(error=status['error']),
            )
            context.logger.debug('iX Portal has disabled API Key: %s', status['error'])
            await set_status(context, TruecommandStatus.FAILED.value)
            ...
            await context.call2(
                context.s.alert.oneshot_create, TruecommandConnectionPendingAlert(error=status['error']),
            )
            context.logger.debug(
                'Pending Confirmation From iX Portal for Truecommand API Key: %s', status['error']
            )

        await asyncio.sleep(POLLING_GAP_MINUTES * 60)
        config = await context.middleware.call('datastore.config', 'system.truecommand')


async def poll_once(context: ServiceContext, config: dict[str, Any]) -> dict[str, Any]:
    response = await post_call(context, payload={
...
async def register_with_portal(context: ServiceContext, config: dict[str, Any]) -> None:
    ...
    response = await post_call(context, payload={
        ...
        'apikey': config['api_key'],
        'nas_pubkey': config['wg_public_key'],
        'hostname': await context.middleware.call('system.hostname'),
        'sysversion': await context.middleware.call('system.version'),
    })
