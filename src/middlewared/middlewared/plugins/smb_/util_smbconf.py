...
EXCLUDED_IDMAP_ITEMS = frozenset(['name', 'range_low', 'range_high', 'idmap_backend'])
...
def generate_smb_conf_dict(
    ...
    if ds_type is DSType.AD:
        smbconf.update({
            ...
            'realm': ds_config['configuration']['domainname'],
            'template homedir': home_path,
            'winbind enum users': enable_backend_cache,
            'winbind enum groups': enable_backend_cache,
        })

        if ds_config['configuration']['idmap']['idmap_backend'] == 'AUTORID':
            idmap_prefix = f'idmap config * :'
            idmap = ds_config['configuration']['idmap']['idmap_domain']
        else:
            builtin = ds_config['configuration']['idmap']['builtin']
            idmap_prefix = f'idmap config {ds_config["configuration"]["domain"]}'
            idmap = ds_config['configuration']['idmap']['idmap_domain']
        ...
        for idmap in ds_config['trusted_domains']:
