...
class InterfaceRenamer:
    ...
    async def commit(self):
        ...
        for vm_device in await self.middleware.call2(
            self.middleware.services.vm.device.query, [["attributes.dtype", "=", "NIC"]]
        ):
            if new_name := self.mapping.get(vm_device.attributes.nic_attach):
