...
class InterfaceService(Service):
    ...
    def configure_bonds(self, sock, parent_interfaces, sync_interface_opts):
        ...
        """
        laggs = self.middleware.call_sync("datastore.query", "network.lagginterface")
        configured = []
        for lagg in laggs:
            name = lagg["lagg_interface"]["int_interface"]
            members = self.middleware.call_sync(
                "datastore.query",
            ...
            try:
                self.configure_bond(
                    sock, lagg, members, parent_interfaces, sync_interface_opts
                )
                configured.append(name)
            except Exception:
                self.logger.error("Error configuring bond %s", name, exc_info=True)

        return configured

    @private
    def configure_bond(
        self, sock, lagg, members, parent_interfaces, sync_interface_opts
    ):
        ...
        """
        name = lagg["lagg_interface"]["int_interface"]
        self.logger.info("Configuring bond %s", name)
        ...
        for member_name in member_names:
            if member_name in sync_interface_opts:
                sync_interface_opts[member_name]["skip_mtu"] = True
            parent_interfaces.append(member_name)

        # Map xmit_hash_policy to enum
        xmit_hash_policy = None
        if lagg.get("lagg_xmit_hash_policy"):
            xmit_map = {
                "LAYER2": BondXmitHashPolicy.LAYER2,
                "LAYER34": BondXmitHashPolicy.LAYER34,
                "LAYER23": BondXmitHashPolicy.LAYER23,
                "ENCAP23": BondXmitHashPolicy.ENCAP23,
                "ENCAP34": BondXmitHashPolicy.ENCAP34,
            }
            xmit_hash_policy = xmit_map.get(lagg["lagg_xmit_hash_policy"].upper())

        # Map lacpdu_rate to enum
        lacpdu_rate = None
        if lagg.get("lagg_lacpdu_rate"):
            lacpdu_map = {
                "SLOW": BondLacpRate.SLOW,
                "FAST": BondLacpRate.FAST,
            }
            lacpdu_rate = lacpdu_map.get(lagg["lagg_lacpdu_rate"].upper())
        ...
        self.logger.debug("Configuring %s with config: %r", name, config)
