...
class TNCRegistrationFinalizeService(Service):
    ...
    async def status_update(self, status: Status, log_message: str | None = None) -> None:
        await set_status(self.context, status.name)
        if log_message:
            logger.error(log_message)

    @job(lock='tnc_finalize_registration')
    async def registration(self, job: Job) -> None:
        ...
        while config.status == Status.REGISTRATION_FINALIZATION_WAITING.name:
            ...
            try:
                ...
                # poll_once needs the raw config dict (with jwt_token etc) for URL composition
                tnc_config = await config_internal(self.context)
                status = await self.poll_once(claim_token, system_id, tnc_config)
            ...
            if status['error'] is None:
                ...
                if 'token' not in status['response']:
                    ...
                    self.middleware.create_task(
                        self.call2(self.s.tn_connect.acme.initiate_cert_generation),
                    )
    ...
    async def poll_once(
        self, claim_token: str, system_id: str, tnc_config: dict[str, Any],
    ) -> dict[str, Any]:
