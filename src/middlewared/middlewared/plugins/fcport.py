                ...
                if chan is not None:
                    if chan.isdigit():
                        ...
                        if int(chan) > npiv_setting:
                            ...
                            verrors.add(
                                f"{schema_name}.port",
                                f"Invalid FC port ({port}) supplied, chan {chan} must be greater than 0"
                                f" and less than NPIV setting {npiv_setting}"
                            )
                    else:
                        verrors.add(
                            f"{schema_name}.port",
        ...
        if target_id is not None:
            try:
                ...
                if target_mode not in ["FC", "BOTH"]:
                    ...
                    verrors.add(
                        f"{schema_name}.target_id",
                        f'Specified target "{target_name}" ({target_id}) does not have a'
                        f' "mode" ({target_mode}) that permits FC access'
                    )
