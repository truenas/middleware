...
class UpdateService(Service):
    ...
    def download(self, job, train):
        ...
        if train is None:
            ...
            train = update_status['status']['current_train']['name']

        location = self.middleware.call_sync('update.get_update_location')

        return self.download_update(job, train, location, 100)

    @private
    def download_update(self, job, train, location, progress_proportion):
        ...
        try:
            ...
            try:
                ...
                train_manifest = self.middleware.call_sync('update.get_train_manifest', train)
                if self.middleware.call_sync('update.can_update_to', train_manifest['version']):
                    ...
                    try:
                        ...
                        if checksum == train_manifest["checksum"]:
                    ...
                    required_size = train_manifest["filesize"] + 500 * 1024 ** 2
                    ...
                    for i in itertools.count(1):
                        with open(dst, "ab") as f:
                            ...
                            try:
                                ...
                                with requests.get(
                                    f"{scale_update_server()}/{train}/{train_manifest['filename']}",
                                    stream=True,
                                    timeout=30,
