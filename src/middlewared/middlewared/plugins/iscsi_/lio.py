...
class iSCSILIOService(Service):
    ...
    def become_active(self):
        ...
        Reads forward_active back to verify the kernel cleared the flag, and
        logs ha_state for post-failover diagnostics.
        """
        fwd_path = LIO_HA_DIR / 'forward_active'
        try:
            fwd_path.write_text('0\n')
        except OSError as e:
            self.logger.error('lio_ha: write forward_active=0 failed', exc_info=True)
            raise CallError(f'LIO failover (forward_active=0) failed: {e}')

        try:
            post_fwd = fwd_path.read_text().strip()
        except OSError as e:
            self.logger.error('lio_ha: cannot read forward_active after write',
                              exc_info=True)
            raise CallError(f'LIO failover verification read failed: {e}')
        ...
        if post_fwd != '0':
            self.logger.error(
                'lio_ha: forward_active=%r after write (expected 0); ha_state=%s',
                post_fwd, ha_state
            )
            raise CallError(
                f'LIO failover did not take effect: forward_active={post_fwd}'
            )

        self.logger.info(
            'lio_ha: become_active complete (forward_active=0, ha_state=%s)',
            ha_state
        )
