...
def validate_extensions(extensions_data: dict[str, Any], schema: str) -> ValidationErrors:
    ...
    for extension in filter(lambda v: v[1]["enabled"], extensions_data.items()):
        ...
        try:
            ...
            verrors.add(f"{schema}.{extension[0]}", f"Please provide valid values for {extension[0]}: {e}")
    ...
    if extensions_data["KeyUsage"]["enabled"] and extensions_data["KeyUsage"]["key_cert_sign"]:
        if not extensions_data["BasicConstraints"]["enabled"] or not extensions_data["BasicConstraints"]["ca"]:
            verrors.add(
                f"{schema}.BasicConstraints", "Please enable ca when key_cert_sign is set in KeyUsage as per RFC 5280."
            )

    if extensions_data["ExtendedKeyUsage"]["enabled"] and not extensions_data["ExtendedKeyUsage"]["usages"]:
        verrors.add(f"{schema}.ExtendedKeyUsage.usages", "Please specify at least one USAGE for this extension.")
