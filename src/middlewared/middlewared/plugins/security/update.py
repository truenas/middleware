...
class SystemSecurityModel(sa.Model):
    ...
    password_complexity_ruleset = sa.Column(sa.Text, nullable=True)
...
class SystemSecurityService(ConfigService):
    ...
    class Config:
        ...
        datastore_extend = 'system.security.extend'

    @private
    async def extend(self, data):
        if data['password_complexity_ruleset'] is None:
            data['password_complexity_ruleset'] = set()
        else:
            data['password_complexity_ruleset'] = set(data['password_complexity_ruleset'].split())
    ...
    async def compress(self, data):
        out = data.copy()
        if data['password_complexity_ruleset']:
            out['password_complexity_ruleset'] = ' '.join(data['password_complexity_rulest'])
        else:
            out['password_complexity_ruleset'] = None
        ...
        if new['enable_gpos_stig']:
            ...
            if new['max_password_age'] > GPOS_STIG_MAX_PASSWORD_AGE:
                raise ValidationError(
                    ...
                    f'Maximum password age must be less than or equal to {GPOS_STIG_MIN_PASSWORD_AGE} days '
                    'in GPOS STIG compatibility mode.'
                )
            ...
            if GPOS_STIG_PASSWORD_COMPLEXITY - new['password_complexity_ruleset']:
                raise ValidationError(
                    'system_security_update.password_history_length',
                    'GPOS STIG compatibility requires the following password complexity '
                    f'rules: {", ".join(GPOS_STIG_PASSWORD_COMPLEXITY)}'
                )
    ...
    async def do_update(self, job, data):
        ...
        await self.middleware.call('datastore.update', self._config.datastore, old['id'], compressed)
