...
class SystemDatasetService(ConfigService):
    ...
    def sysdataset_path(self, expected_datasetname=None):
        ...
        if expected_datasetname is None:
            ...
            pool = db_pool or self.middleware.call_sync('boot.pool_name')
    ...
    async def config_extend(self, config):
        ...
        config['pool'] = config['pool'] or await self.middleware.call('boot.pool_name')
    ...
    async def do_update(self, job, data):
        ...
        # Perform migration if pool changed
        if old_pool != new_pool:
            self.middleware.call_sync('systemdataset.migrate', old_pool, new_pool)
        else:
            # Just ensure everything is set up correctly
            self.middleware.call_sync('systemdataset.setup', data['pool_exclude'])
    ...
    async def _validate_and_select_pool(self, data, config):
        ...
        if new_pool:
            ...
            for pool in await self.middleware.call(
                ...
                # No data pool found, use empty string (will resolve to boot pool)
                new_pool = ''
    ...
    def setup_impl(self, exclude_pool):
        ...
        if target_pool != config['pool'] and not is_fallback:
            ...
            job = self.middleware.call_sync('systemdataset.update', {'pool': target_pool})
    ...
    def _pool_is_available(self, pool):
        ...
        # Check if pool root is mounted (walk incrementally and break early)
        pool_mounted = any(
    ...
    def apply_dataset_spec(self, datasets):
        for ds_config in datasets:
            ...
            mountpoint = ds_config.get('mountpoint', f'{SYSDATASET_PATH}/{name}')
    ...
    def _atomic_remount(self, new_path, target_path, new_fd):
        ...
        for mnt in iter_mountinfo(path=new_path, reverse=True):
            truenas_os.umount2(mnt['mountpoint'], truenas_os.MNT_DETACH)
    ...
    def _post_mount_setup(self):
        ...
        if os.path.exists(corepath):
            ...
            truenas_os.umount2('/var/lib/systemd/coredump', truenas_os.MNT_DETACH)
...
async def setup(middleware):
    def setup_paths():
        os.makedirs(SYSDATASET_PATH, mode=0o755, exist_ok=True)
        truenas_os.mount_setattr('/var', propagation=truenas_os.MS_PRIVATE, flags=truenas_os.AT_RECURSIVE)
