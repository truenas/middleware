...
class SystemGeneralService(ConfigService):
    ...
    async def do_update(self, data):
        ...
        ui_restart_delay = data.pop('ui_restart_delay')

        if rollback_timeout is not None and rollback_timeout <= ui_restart_delay:
            verrors = ValidationErrors()
            verrors.add(
                'system_general_update',
                'rollback_timeout must be strictly greater than ui_restart_delay.'
            )
            raise verrors
