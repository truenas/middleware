...
def query_impl(hdl, data):
    ...
    if state.query_args["paths"]:
        ...
        # specifying any paths, then we'll be nice and
        # set get_children: True so that it returns
        # some information. Otherwise it's a less than
        # stellar experience
        state.query_args["get_children"] = (
            state.query_args["get_children"] or not state.query_args["paths"]
        )
