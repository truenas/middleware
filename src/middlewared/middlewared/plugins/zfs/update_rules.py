"""Validation rules for zfs.resource.update.

Same shape as create_rules: each rule takes the request model and an
UpdateContext of resolved values and gathered facts, raises a single
ValidationError on a failed check, and returns nothing otherwise. Rules
never perform I/O. The checks shared with create come from create_rules
as `reject_*` functions taking plain values; the rules here resolve
"effective" from the resource being updated rather than from an
ancestor.
"""
...
def check_path_shape(data: ZFSResourceUpdateArgsData, ctx: UpdateContext) -> None:
    """A snapshot path is not a resource."""
    if "@" in data.path:
...
def check_has_work(data: ZFSResourceUpdateArgsData, ctx: UpdateContext) -> None:
    """The request must change something."""
...
def check_set_inherit_conflict(data: ZFSResourceUpdateArgsData, ctx: UpdateContext) -> None:
    """A property may be set or inherited in one request, not both."""
...
def _effective_value(name: str, ctx: UpdateContext) -> str | None:
    ...
    requested value or the resource's current value."""
    assert ctx.current is not None
...
def check_acl_combination(data: ZFSResourceUpdateArgsData, ctx: UpdateContext) -> None:
    ...
    aclmode and after the resource has been read.
    """
...
def check_dedup_tiering(context: ServiceContext, data: ZFSResourceUpdateArgsData, ctx: UpdateContext) -> None:
    ...
    The effective special_small_blocks is the requested value or the
    resource's current one.

    The service calls this only for filesystems that request a dedup
    value other than off while tiering is enabled and after the resource
    has been read.
    """
    assert ctx.current is not None
