...
from middlewared.service import CallError, ConfigService, ConfigServicePart, ValidationError, private
...
class ZfsTierRewriteJobStatusEventSource(EventSource):
    """
    ...
    def run_sync(self) -> None:
        tier_job_id = typing.cast(dict[str, typing.Any], self.arg)["tier_job_id"]
        last_info = None
...
class ZfsTierRewriteJobQueryEventSource(EventSource):
    """
...
class ZfsTierService(ConfigService[ZfsTierEntry]):
    class Config:
        namespace = "zfs.tier"
        cli_private = True
        role_prefix = "DATASET"
    ...
    async def config(self) -> ZfsTierEntry:
...
async def setup(middleware: Middleware) -> None:
    # Registered first: everything below can raise, and a delegate that never registers fails
    # open -- the config keeps whatever the last render said, with no symptom anywhere.
