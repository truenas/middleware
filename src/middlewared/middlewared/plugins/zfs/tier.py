...
from middlewared.service import CallError, ConfigService, private, ValidationError
...
def _map_create_result(result) -> dict:
...
def _map_recover_result(result) -> dict:
...
def _bulk_tier_map(lzh, dataset_names: list[str]) -> dict[str, dict | None]:
    """
...
def get_dataset_tier_info_cached(
    ...
    try:
        dprops = hdl.get_properties(properties={
    ...
    def run_sync(self):
        ...
        while not self._cancel_sync.is_set():
            ...
            for tier_job_id, status in current.items():
                ...
                if prev is None:
                    if not first or status in (RewriteJobStatus.RUNNING, RewriteJobStatus.QUEUED):
...
class ZfsTierService(ConfigService):
    ...
    @api_method(ZfsTierUpdateArgs, ZfsTierUpdateResult, roles=['DATASET_WRITE'],
                audit='ZFS tier config update')
    async def do_update(self, data):
        if not await self.middleware.call('system.is_enterprise'):
            raise CallError('ZFS tiering requires a TrueNAS Enterprise license.')

        old = await self.config()
        new = {**old, **data}
        await self.middleware.call('datastore.update', 'zfs.tier', new['id'], new)
        await self.middleware.call('etc.generate', 'truenas_zfsrewrited')
        if new['enabled'] != old['enabled']:
            verb = 'START' if new['enabled'] else 'STOP'
            await self.middleware.call('service.control', verb, 'truenas_zfsrewrited')
        return await self.config()

    @api_method(ZfsTierRewriteJobCreateArgs, ZfsTierRewriteJobCreateResult, roles=['DATASET_WRITE'],
                audit='ZFS tier rewrite job create', audit_extended=lambda data: data['dataset_name'])
    async def rewrite_job_create(self, data):
    ...
    async def rewrite_job_query(self, data):
    ...
    @api_method(ZfsTierRewriteJobStatusArgs, ZfsTierRewriteJobStatusResult, roles=['DATASET_READ'])
    async def rewrite_job_status(self, data):
        ...
        try:
            info = client.get_info(dataset_name, job_uuid)
        except Exception as e:
            raise CallError(f'Failed to get job info: {e}')
        return _map_info_result(info)

    @api_method(ZfsTierRewriteJobFailuresArgs, ZfsTierRewriteJobFailuresResult, roles=['DATASET_READ'])
    ...
    async def rewrite_job_recover(self, data):
    ...
    @private
    def _validate_tier_space(self, field: str, dataset_name: str, target_tier: str, space_info: dict) -> None:
    ...
    async def dataset_set_tier(self, data):
        ...
        if tier_job and tier_job['status'] in (RewriteJobStatus.RUNNING, RewriteJobStatus.QUEUED):
            raise ValidationError(
                field,
        ...
        return new_tier_info

    @private
    async def get_tier_space_info(self, dataset_name: str) -> dict | None:
        """
        ...
        try:
            pools = await self.call2(self.s.zpool.query_impl, {
                'pool_names': [pool_name],
                'properties': [
                    'class_special_usable', 'class_special_used',
                    'class_normal_usable', 'class_normal_used',
                ],
            })
            if not pools:
                return None
    ...
    def bulk_get_tier_info(self, tls, dataset_names: list[str]) -> dict[str, dict | None]:
        """
