...
if TYPE_CHECKING:
    from middlewared.api.current import PoolDatasetLockOptions, PoolDatasetUnlockOptions
...
async def lock_impl(ctx: ServiceContext, job: Job, id_: str, options: PoolDatasetLockOptions) -> Literal[True]:
    ...
    await ctx.call2(ctx.s.pool.dataset.stop_attachment_delegates, mountpoint)

    # recursive doesn't apply to zvols
    recursive = ds.type != 'VOLUME'
    await ctx.call2(
        ctx.s.zfs.resource.unload_key,
        id_,
        recursive=recursive,
    ...
    if ds.mountpoint:
        await ctx.middleware.call('filesystem.set_zfs_attributes', {
            'path': ds.mountpoint,
            'zfs_file_attributes': {'immutable': True}
        })

    await ctx.middleware.call_hook('dataset.post_lock', id_)

    return True


def unlock_impl(ctx: ServiceContext, job: Job, id_: str, options: PoolDatasetUnlockOptions) -> dict:
    verrors = ValidationErrors()
    dataset = ctx.middleware.call_sync('pool.dataset.get_instance', id_)
    keys_supplied = {}
    ...
    for i, ds in enumerate(options.datasets):
        ...
        elif not ds_key and not ds_passphrase:
            verrors.add(
        ...
        if not options.force and not ds.force:
            ds_quick = ctx.call_sync2(ctx.s.pool.dataset.get_instance_quick, ds.name, {'encryption': True})
            if ds_quick.locked:
        ...
        keys_supplied[ds.name] = ds_key or ds_passphrase
    ...
    if '/' in id_ or not options.recursive:
        if not dataset['locked']:
            ...
            if not bool(
                ctx.middleware.call_sync('pool.dataset.query_encrypted_roots_keys', [['name', '=', id_]])
    ...
    locked_datasets = []
    datasets = ctx.middleware.call_sync(
        'pool.dataset.query_encrypted_datasets', id_.split('/', 1)[0], {'key_loaded': False}
    )
    assign_supplied_recursive_keys([ds.model_dump() for ds in options.datasets], keys_supplied, list(datasets.keys()))
    ...
    for name_i, name in enumerate(names):
        ...
        for ds in to_mount:
            ...
            if os.path.exists(mount_path):
                ...
                try:
                    sfs = ctx.middleware.call_sync('filesystem.statfs', mount_path)
                except Exception:
                ...
                try:
                    ctx.middleware.call_sync('filesystem.set_zfs_attributes', {
                        'path': mount_path,
                ...
                if not os.path.isdir(mount_path) or not directory_is_empty(mount_path):
                    ...
                    try:
                        ...
                        ctx.logger.error('%s: failed to move unexpected directory or file from mount path',
            ...
            try:
                ctx.call_sync2(ctx.s.zfs.resource.mount, ds['name'])
            except Exception as e:
    ...
    if unlocked:
        if options.toggle_attachments:
            ...
            ctx.call_sync2(ctx.s.pool.dataset.unlock_handle_attachments, dataset)
        ...
        for unlocked_dataset in filter(lambda d: d in keys_supplied, unlocked):
            ...
            # TODO: Convert to call_sync2 once insert_or_update_encrypted_record is refactored
            ctx.middleware.call_sync(
...
def assign_supplied_recursive_keys(request_datasets: list[dict], keys_supplied: dict, queried_datasets: list[str]) -> None:
    """Recursively assign keys to child datasets based on parent recursive flag."""
    request_datasets_dict = {ds['name']: ds for ds in request_datasets}
    for name in queried_datasets:
        if name not in keys_supplied:
            for parent in Path(name).parents:
                parent = str(parent)
                if parent in request_datasets_dict and request_datasets_dict[parent]['recursive']:
                    if parent in keys_supplied:
                        keys_supplied[name] = keys_supplied[parent]
                        break


async def unlock_handle_attachments(ctx: ServiceContext, dataset: dict) -> None:
    mountpoint = dataset_mountpoint(dataset)
    for attachment_delegate in await ctx.call2(ctx.s.pool.dataset.get_attachment_delegates_for_start):
        # FIXME: put this into `VMFSAttachmentDelegate`
        if attachment_delegate.name == 'vm':
            await ctx.middleware.call('pool.dataset.restart_vms_after_unlock', dataset)
            continue
