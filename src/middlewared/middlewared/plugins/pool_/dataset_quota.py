...
def quota_cb(quota, state):
    if quota.quota_type in (
        ...
        # shouldn't be reachable but return early to be safe
        return True
...
def query_for_quota_alert(tls: threading.local) -> dict[str, dict]:
    """Called exclusively by alert system to inform users of quota thresholds."""
    state = {"pools": dict(), "datasets": dict()}
    tls.lzh.iter_root_filesystems(callback=quota_alert_cb, state=state)
    return state


def get_quota_impl(tls: threading.local, ds: str, quota_type: str) -> list[dict]:
...
async def get_quota(ctx: ServiceContext, ds: str, quota_type: str, filters: list, options: dict) -> list[dict]:
    # Call impl via middleware since it needs threading.local from @pass_thread_local_storage
    quota_list = await ctx.middleware.call('pool.dataset.get_quota_impl', ds, quota_type)
    return filter_list(quota_list, filters, options)


def set_quota_impl(tls: threading.local, ds: str, inquotas: list[dict]) -> None:
...
async def set_quota(ctx: ServiceContext, ds: str, data: list[dict]) -> None:
    quotas = []
    for i, q in enumerate(data):
        ...
        if q['quota_type'] == 'DATASET':
            ...
            if not q['id'].isdigit():
                try:
                    xid = (await ctx.middleware.call(
                        f'{id_type}.get_{id_type}_obj',
                        {f'{id_type}name': q['id']}
                    ))['pw_uid' if id_type == 'user' else 'gr_gid']
                except Exception:
                    ctx.logger.debug('Failed to convert %s [%s] to id.', id_type, q['id'], exc_info=True)
    ...
    if quotas:
        # Call impl via middleware since it needs threading.local from @pass_thread_local_storage
        await ctx.middleware.call('pool.dataset.set_quota_impl', ds, quotas)
