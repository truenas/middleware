...
class ContainerConfigService(ConfigService):
    ...
    async def do_update(self, data):
        ...
        if config["image_dataset"] is not None:
            if not await self.middleware.call(
                "pool.dataset.query", [["name", "=", config["image_dataset"]], ["type", "=", "FILESYSTEM"]]
            ):
                verrors.add("container_config_update.image_dataset", "The dataset must exist and be a filesystem.")
