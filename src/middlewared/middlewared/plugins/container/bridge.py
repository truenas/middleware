...
class ContainerService(Service):
    ...
    def configure_bridge(self):
        ...
        with open(f"{FS_PATH}/test.dnsmasq.raw", "w") as f:
            pass
        ...
        subprocess.Popen(
            ["dnsmasq", "--keep-in-foreground", "--strict-order", "--bind-interfaces", "--except-interface=lo",
             ...
             "-s", DOMAIN_NAME, "--interface-name", f"_gateway.{DOMAIN_NAME},{BRIDGE_NAME}", "-S", f"/{DOMAIN_NAME}/",
             f"--conf-file={FS_PATH}/test.dnsmasq.raw", "-u", "nobody", "-g", "nogroup"],
