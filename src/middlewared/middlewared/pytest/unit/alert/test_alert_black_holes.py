...
every ``*Alert`` name appearing anywhere in a source's body is an edge, reachable or not. A
...
def _alert_class_reference(node: ast.AST) -> str | None:
    ...
    if name.endswith("Alert") and name not in FRAMEWORK_NAMES:
        return name
...
def _classes_a_source_may_create() -> dict[str, set[str]]:
    ...
    base class the way `AlertService.load` does.
    """
        ...
        for node in ast.walk(tree):
            ...
            for child in ast.walk(node):
                if (referenced := _alert_class_reference(child)) is not None:
                    names.add(referenced.removesuffix("Alert"))
