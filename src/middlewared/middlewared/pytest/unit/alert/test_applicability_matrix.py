...
ALERT_CLASS_BASES = frozenset({"AlertClass", "OneShotAlertClass", "DismissableAlertClass"})

UNIT_TEST_DIR = os.path.join(get_middlewared_dir(), "pytest") + os.sep


def _declared_by_the_unit_tests(declaration) -> bool:
    """Whether `declaration` was written by a test rather than by the product.

    Keyed on the file it came from, not on `__module__`: a test module whose directory carries no
    ``__init__.py`` is imported under a bare top-level name, so it has no dotted prefix to match.
    """
    try:
        return inspect.getfile(declaration).startswith(UNIT_TEST_DIR)
    except TypeError:
        return False
...
def _modules_declaring_alert_classes() -> list[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for filename in filenames:
            ...
            if any(
                isinstance(node, ast.ClassDef) and any(_base_name(base) in ALERT_CLASS_BASES for base in node.bases)
...
def test_the_scan_reaches_every_module_that_declares_a_class():
    ...
    registered = {klass.__module__ for klass in AlertClass.classes if not _declared_by_the_unit_tests(klass)}
...
def declarations():
    ...
    Declarations made by the unit-test tree are left out. They register with the same metaclass as
    the rest, so whether they are present depends on what else the session imported, and the
    inventory has to be the same answer whether this module is run alone or with everything.
    """
    sources = [
        (cls.__name__.removesuffix("AlertSource"), "source", cls)
        ...
        if not _declared_by_the_unit_tests(cls)
    ]
...
def rule_of(kind: str, declaration, attribute: str):
    """A declaration's rule, read where its kind keeps it.

    A class states its rules in its `AlertClassConfig`; a source states them on itself. Reading a
    class as if it were a source finds nothing and reports every class as unruled.
    """
    holder = declaration if kind == "source" else declaration.config
...
def test_every_declaration_carries_a_rule():
    ...
    for (name, kind), cells in inventory.items():
        ...
        rule = rule_of(kind, declaration, "applies_to")
        if kind == "listed":
            rule = rule or rule_of(kind, declaration, "listed_only_when")
...
def test_listed_only_when_hides_without_silencing(class_name):
    ...
    assert applies(klass.config.applies_to, facts) is True
...
def test_every_rule_is_a_vocabulary_name():
    ...
    for name, kind, declaration in declarations():
        for attribute in ("applies_to", "listed_only_when"):
            rule = rule_of(kind, declaration, attribute)
...
def test_ha_classes_are_not_listed_without_an_ha_license():
    ...
    listed = [
        (klass.config.name, population.name)
        for klass in AlertClass.classes
        if klass.config.category is AlertCategory.HA
...
def test_the_flag_carriers_are_what_was_reviewed():
    ...
    live = sorted(
        (name, flag, source_rule_name(source))
