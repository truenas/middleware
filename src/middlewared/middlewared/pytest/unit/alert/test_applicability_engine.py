...
def stub_config(applies_to=None, listed_only_when=None):
    return AlertClassConfig(
        category=AlertCategory.SYSTEM,
        level=AlertLevel.WARNING,
        title="Stub",
        applies_to=applies_to,
        listed_only_when=listed_only_when,
    )
...
class HardwareOnlyDeclaration:
    config = stub_config(applies_to=TRUENAS_HARDWARE)
    applies_to = TRUENAS_HARDWARE


class NarrowedDeclaration:
    config = stub_config(applies_to=TRUENAS_HARDWARE, listed_only_when=HA_LICENSED)
    applies_to = TRUENAS_HARDWARE
...
def test_rule_name_reports_the_population_a_declaration_named():
    ...
    assert class_rule_name(HardwareOnlyDeclaration) == "TRUENAS_HARDWARE"
    assert source_rule_name(HardwareOnlyDeclaration) == "TRUENAS_HARDWARE"
    assert class_rule_name(UnconstrainedDeclaration) == "unconstrained"
    assert source_rule_name(UnconstrainedDeclaration) == "unconstrained"
...
def test_applicability_evaluates_each_declaration_once():
    ...
    class Declared:
        config = stub_config(applies_to=COUNTED)
        applies_to = COUNTED
...
def test_distinct_facts_give_distinct_answers(hardware_class, license, expected):
    ...
    class HaDeclaration:
        config = stub_config(applies_to=HA_LICENSED)
...
def test_a_rule_is_read_off_the_class_never_an_instance():
    """``state.alert_sources`` holds instances, which is why ``source_runs`` takes the class."""
    facts = make_facts(hardware_class=HardwareClass.TRUENAS_HW, license=NO_LICENSE)
