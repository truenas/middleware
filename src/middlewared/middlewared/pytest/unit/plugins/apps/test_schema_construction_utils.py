...
def test_immutable_fields_in_list_items():
    """Test immutable fields within list items"""
    schema = [
        {
    ...
    # Create mode - can set any values
    model_create = generate_pydantic_model(schema, 'TestListImmutableCreate', NOT_PROVIDED, NOT_PROVIDED)
    m1 = model_create(additional_storage=[
        {'type': 'host_path', 'mount_path': '/mnt/data1'},
        {'type': 'ix_volume', 'mount_path': '/mnt/data2'}
    ])
    assert len(m1.additional_storage) == 2
    assert m1.additional_storage[0].type == 'host_path'
    assert m1.additional_storage[1].type == 'ix_volume'

    # Update mode - immutable fields should be locked
    old_values = {
        'additional_storage': [
            {'type': 'host_path', 'mount_path': '/mnt/data1'},
            {'type': 'ix_volume', 'mount_path': '/mnt/data2'}
    ...
    model_update = generate_pydantic_model(schema, 'TestListImmutableUpdate', NOT_PROVIDED, old_values)
    # This should fail because we're trying to change an immutable field
    with pytest.raises(ValidationError) as exc_info:
        model_update(additional_storage=[
            {'type': 'temp_path', 'mount_path': '/mnt/data1'},  # Changed immutable type field
            {'type': 'ix_volume', 'mount_path': '/mnt/data2'}   # Kept same
        ])
