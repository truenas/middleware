...
class TestVendorRestartCoalescing:
    ...
    @pytest.mark.asyncio
    async def test_sequential_requests_each_restart(self, restart_env):
        middleware, run_mock = restart_env
    ...
    @pytest.mark.asyncio
    async def test_failed_restart_leaves_clean_state(self, restart_env):
        """A restart failure releases the lock and clears the queued flag."""
        middleware, run_mock = restart_env
        run_mock.side_effect = [RuntimeError("systemctl failed"), None]
