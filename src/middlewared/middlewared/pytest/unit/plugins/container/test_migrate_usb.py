...
def make_service():
    ...
    middleware["container.device.nic_attach_choices"] = AsyncMock(return_value={"BRIDGE": [], "MACVLAN": []})
    middleware["container.device.gpu_choices"] = AsyncMock(return_value={})
    middleware["datastore.insert"] = AsyncMock(return_value=1)
    return create_service(middleware, ContainerService), middleware


def inserted_devices(middleware):
    return [call.args[1] for call in middleware["datastore.insert"].call_args_list]
...
async def test_migrate_devices_never_scans_udev_itself(monkeypatch, connected):
    ...
    service, middleware = make_service()
    job = FakeJob()

    for container_id, name in ((1, "first"), (2, "second")):
        await service.migrate_devices(job, manifest, {"id": container_id, "name": name}, connected)
    ...
    assert inserted_devices(middleware) == [
...
async def test_migrate_uses_one_scan_for_every_pool(monkeypatch):
    ...
    service, middleware = make_service()
    middleware["datastore.query"] = AsyncMock(return_value=[{"id": 1, "pool": "tank", "storage_pools": "dozer"}])
    middleware["container.license_active"] = AsyncMock(return_value=True)
    middleware["container.query"] = AsyncMock(return_value=[])
    middleware["container.migrate_specific_pool"] = AsyncMock()

    await service.migrate(FakeJob())

    assert scans == 1
    passed = [call.args[-1] for call in middleware["container.migrate_specific_pool"].call_args_list]
    assert len(passed) == 2
...
async def test_a_device_without_both_ids_is_skipped_and_said_so(connected, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="ContainerService"):
        await service.migrate_devices(job, manifest, {"id": 1, "name": "first"}, connected)
...
async def test_how_each_device_resolved_is_recorded(connected, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="ContainerService"):
        await service.migrate_devices(FakeJob(), manifest, {"id": 1, "name": "first"}, connected)
