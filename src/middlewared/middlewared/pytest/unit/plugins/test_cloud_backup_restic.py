...
@pytest.mark.asyncio
async def test_malformed_error_does_not_kill_reader_and_logs_once():
    # An unexpected message shape (e.g. restic's pre-0.17.1 empty `{}`) must not
    # raise out of the reader task (which would stop draining stdout and hang the
    # job). It is recorded and the traceback is logged at most once per run.
    job = make_job()
    bad = line({"message_type": "error", "error": {}, "during": "archival", "item": "/x"})
    await restic_check_progress(job, make_proc([bad, bad]), track_progress=True)
...
@pytest.mark.asyncio
async def test_status_message_updates_progress():
    job = make_job()
    await restic_check_progress(
        job,
