...
@pytest.mark.parametrize('override,rejected_field,fragment', [
    (
        {'bootloader': 'UEFI_CSM'},
...
async def test_aarch64_rejects_x86_only_flags(override, rejected_field, fragment):
    ...
    with (
        ...
        patch('middlewared.plugins.vm.crud.guest_architecture_and_machine_choices',
              return_value=_ARCH_CHOICES),
        patch('middlewared.plugins.vm.crud.cpu_model_choices',
              return_value={'cortex-a53': 'cortex-a53'}),
    ):
    ...
    assert matching, (
        f"Expected error on {rejected_field!r}; got: {[(e.attribute, e.errmsg) for e in verrors.errors]}"
    )
    assert fragment in matching[0].errmsg, (
        f"Expected {fragment!r} in error message, got: {matching[0].errmsg!r}"
    )


@pytest.mark.parametrize('flag_override', [
    {'bootloader': 'UEFI_CSM'},
    {'hyperv_enlightenments': True},
    {'hide_from_msr': True},
])
...
@pytest.mark.parametrize('host_arch,guest_arch,cpu_mode,should_error', [
...
async def test_cross_arch_cpu_mode(host_arch, guest_arch, cpu_mode, should_error):
    ...
    if should_error:
        assert cpu_errors, (
            f"Expected cpu_mode error for {cpu_mode!r} "
            f"(guest={guest_arch!r}, host={host_arch!r}); no error raised"
        )
