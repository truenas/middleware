...
def test_empty_cached_entry_is_treated_as_a_miss_and_popped():
    context = make_context(cached={})

    assert apps(context, CatalogApps(cache=True, cache_only=True)).root == {}

    assert ("cache.pop", CACHE_KEY) in [call.args for call in context.middleware.call_sync.call_args_list]
...
def test_populated_cached_entry_is_a_hit():
    ...
    assert "cache.pop" not in called_methods(context)
...
@pytest.mark.parametrize("other_location", ["/var/run/middleware/ix-apps/catalogs", "/mnt/other/ix-apps/catalogs"])
def test_cache_key_distinguishes_catalog_locations(other_location):
    assert get_cache_key("TRUENAS", LOCATION) != get_cache_key("TRUENAS", other_location)
