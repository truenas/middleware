...
def make_delegate(name, etc_groups, order=0, **attrs):
    return type(
        ...
        (LicenseReconcileDelegate,),
        {
            ...
            "order": order,
            "reason": f"{name} is license sensitive",
            **attrs,
        },
    )()
...
@pytest.mark.asyncio
async def test_reconcile_without_a_license_does_nothing():
    service, middleware, rendered = reconcile_service(license_info=None)
    ...
    await service.register_reconcile_delegate(make_delegate("smb", ("smb",), should_run=should_run))
...
@pytest.mark.asyncio
async def test_reconcile_without_a_license_terminates_progress():
    """The job has to come to rest even when the pass declines to do anything at all."""
    service, middleware, rendered = reconcile_service(license_info=None)
    job = FakeJob()
    ...
    assert job.progress == [(100, "No license could be read; nothing was reconciled")]
