...
from middlewared.plugins.support.execute import ADDRESS
...
def posted_urls(monkeypatch):
    """Record every URL the support plugin would have posted to, and post to none of them."""
    urls = []
    ...
    monkeypatch.setattr("middlewared.plugins.support.execute.post", fake_post)
    monkeypatch.setattr("middlewared.plugins.support.execute.requests", _FakeRequests(urls))
    monkeypatch.setattr("middlewared.plugins.support.execute.sw_version", lambda: "TrueNAS-26.04.0")
    return urls
...
def _service(column):
    m = Middleware()
    m.services.system.vendor.name = lambda *args: None
