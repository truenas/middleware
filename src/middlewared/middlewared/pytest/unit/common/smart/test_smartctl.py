...
async def test__get_smartctl_args__disk_nonexistent():
    with patch("middlewared.common.smart.smartctl.IS_LINUX", Mock(return_value=False)):
...
async def test__get_smartctl_args__nvme():
    with patch("middlewared.common.smart.smartctl.IS_LINUX", Mock(return_value=True)):
