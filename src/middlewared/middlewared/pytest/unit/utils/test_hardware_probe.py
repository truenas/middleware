...
def wire(monkeypatch):
    ...
    def build(chassis: str, detect):
        monkeypatch.setattr(probe, "cached_dmi", lambda: SimpleNamespace(tn_model=chassis))
...
def test_hardware_half_reaches_ha_platform(wire):
    wire("TRUENAS-M50", lambda: ("ECHOWARP", "B"))
