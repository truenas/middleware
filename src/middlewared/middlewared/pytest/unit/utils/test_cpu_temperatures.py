...
# ---------------------------------------------------------------------------
# 1. Intel single-package, HT, grouped enumeration
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. Intel dual-socket - 'Package id N' label drives package mapping
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. AMD Ryzen 5 3600, single CCD, Tccd1 wins for all cores
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. AMD Threadripper 1950X (AMD_PREFER_TDIE) - Tdie wins over Tccd
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. AMD_PREFER_TDIE without Tdie cascades to Tctl
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7. AMD multi-CCD fully populated - Tccd1 -> die 0, Tccd2 -> die 1
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 8. AMD asymmetric CCDs - die 0 has 4 cores, die 1 has 8 (no modulo gate)
# ---------------------------------------------------------------------------


def test_amd_asymmetric_ccds():
    """A 12-core chip with one fully-fused-off-half CCD: die 0 has 4 phys
    cores, die 1 has 8. The old `core_count % len(ccds) == 0` modulo gate
    would have refused this layout; per-die assignment handles it natively."""
...
# ---------------------------------------------------------------------------
# 9. AMD with Tccd2 missing - die 1 cores fall back to Tctl
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 10. Ryzen 7 5825U - the user-reported 1.5x bug, must be 1.0x now
# ---------------------------------------------------------------------------
...
def test_amd_multi_socket_distinct_packages():
    """Two k10temp chips with distinct package_ids. Neither should
    overwrite the other (today's blind .update() does)."""
    cinfo = make_cinfo(
...
# ---------------------------------------------------------------------------
# 12. AMD multi-socket with package_id resolution failure (alphabetical fallback)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 13. AMD Opteron k8temp legacy - temp1 broadcast across the package
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 14. 4-way SMT (POWER) - all 4 logical CPUs get the temp
# ---------------------------------------------------------------------------


def test_4way_smt_all_siblings_populated():
    """A POWER-style chip with core_cpus_list = '0-3' (4-way SMT). With the
    old ht_map mirror loop only members[1] would be filled; with logical_to_phys
    every sibling gets the temp."""
    cinfo = make_cinfo(
...
# ---------------------------------------------------------------------------
# 15. Partial SMT - some HT siblings offline; logical_to_phys covers truth
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 16. Fallback when no chips are present
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 17. 0.0 readings preserved on every AMD branch
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 18. Tccd1..Tccd12 with scrambled insertion order
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 19. core_count / physical_core_count public invariants
# ---------------------------------------------------------------------------
