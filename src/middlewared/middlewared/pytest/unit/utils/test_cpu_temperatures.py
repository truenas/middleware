...
@patch('middlewared.utils.cpu.SensorsWrapper')
def test_read_temps_multiple_chips(mock_sensors_wrapper):
    """Test reading temperatures from dual socket Intel system"""
    mock_sensors = MagicMock()
    mock_sensors_wrapper.return_value.__enter__.return_value = mock_sensors
...
@patch('middlewared.utils.cpu.SensorsWrapper')
def test_read_temps_error_handling(mock_sensors_wrapper):
    """Test that read_cpu_temps handles errors gracefully"""
    mock_sensors_wrapper.return_value.__enter__.side_effect = OSError('Could not find libsensors')
