...
def dmi(monkeypatch):
    def build(**kwargs):
        system = SimpleNamespace(**{"product_name": "", "manufacturer": "", "serial_number": "", **kwargs})
        monkeypatch.setattr(detect, "cached_dmi", lambda: SimpleNamespace(system=system))
...
def test_dmi_without_a_system_record(monkeypatch):
    """DMI that carries no Type 1 structure at all."""
    monkeypatch.setattr(detect, "cached_dmi", lambda: SimpleNamespace(system=None))
...
def test_no_product_name_beats_the_qemu_stamp(dmi):
    ...
    dmi(manufacturer="QEMU", product_name="", serial_number="ha1")
    assert detect.detect_platform() == ("MANUAL", "MANUAL")
...
def test_qemu_ha_serials(dmi, serial, node):
    dmi(manufacturer="QEMU", product_name="Standard PC", serial_number=serial)
...
def test_qemu_stamp_wins_over_a_truenas_product_name(dmi):
    dmi(manufacturer="QEMU", product_name="TRUENAS-M50", serial_number="ha1")
...
def test_non_platform_prefix_bails_early(dmi, monkeypatch, product):
    ...
    dmi(manufacturer="Supermicro", product_name=product)
    assert detect.detect_platform() == ("MANUAL", "MANUAL")
