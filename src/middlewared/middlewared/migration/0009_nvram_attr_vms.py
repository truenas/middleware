...
from middlewared.plugins.vm.utils import FINAL_NVRAM_FOLDER_PATH, get_vm_nvram_file_name
...
def migrate(middleware):
    os.makedirs(FINAL_NVRAM_FOLDER_PATH, exist_ok=True)
    os.chown(FINAL_NVRAM_FOLDER_PATH, LIBVIRT_QEMU_UID, LIBVIRT_QEMU_GID)

    if middleware.call_sync('system.is_ha_capable'):
        middleware.logger.error('Skipping nvram migration as system is HA capable')
        return
...
def migrate_vm_nvram_file(middleware, vm):
    ...
    new_path = os.path.join(FINAL_NVRAM_FOLDER_PATH, file_name)
