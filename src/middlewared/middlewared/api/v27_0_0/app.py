...
class AppEntry(BaseModel):
    ...
    latest_version: NonEmptyString | None = Field(
        description="The latest available version string, or `null` if unable to determine latest version.",
    )
    latest_app_version: NonEmptyString | None = Field(
        description="The latest application version string, or `null` if unable to determine latest version.",
    )
...
class AppCertificateChoicesResult(BaseModel):
    result: AppCertificateChoices = Field(
...
class AppIpChoicesResult(BaseModel):
    result: AppIpChoices = Field(description="Object mapping IP addresses to their descriptive names.")
...
class AppUpgradeBulkArgs(BaseModel):
    apps: list[AppUpgradeBulkEntry] = Field(description="List of applications to upgrade, each with its own options.")


class AppBulkUpgradeJobResult(BaseModel):
    app_name: NonEmptyString = Field(description="Name of the application.")
    error: LongString | None = Field(description="Error message if the upgrade failed, or `null` on success.")
    result: AppEntry | None = Field(description="The application entry after upgrade, or `null` if the upgrade failed.")


class AppUpgradeBulkResult(BaseModel):
    result: list[AppBulkUpgradeJobResult] = Field(
        description="Per-app upgrade results in the same order as the input list.",
    )
