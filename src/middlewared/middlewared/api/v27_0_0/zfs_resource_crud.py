...
class ZFSResourceEntry(BaseModel):
    ...
    properties: ZFSPropertiesEntry | None = Field(description="The zfs properties for the resource.")
...
class ZFSResourceQuery(BaseModel):
    paths: UniqueList[str] | set[str] = Field(
        default=list(),
        description=(
    ...
    max_depth: int = Field(
        ...
        description=(
            ...
            "\n"
            "Note: When max_depth > 0 is specified, it takes priority over get_children. The depth is measured from the"
...
class ZFSResourceUpdateProperties(ZFSResourceCreateProperties):
    ...
    Each field is the native ZFS property name and values are handed to ZFS verbatim.
    A field left as null is not touched. Properties that are fixed at creation time are not accepted.
    Fields marked `Private` are settable by internal callers only.
    """
...
class ZFSResourceDestroyArgs(BaseModel):
    data: ZFSResourceDestroyArgsData = Field(description="Parameters.")
