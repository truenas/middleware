...
class FCPortStatusOptionsExtra(BaseModel):
    with_lun_access: bool = Field(
        default=True,
        description=(
            "When `true`, only include Fibre Channel sessions that have access to at least one LUN (Logical Unit "
            "Number). When `false`, include all Fibre Channel sessions regardless of LUN access."
        ),
    )
