...
class CoreDownloadArgs(BaseModel):
    method: str = Field(examples=["config.save"], description="Name of the job method to execute.")
    args: list = Field(description="Arguments to pass to the job method.")
    filename: str = Field(
        examples=["system-config-backup.db"],
        description="Suggested filename for the downloaded file. Sets the `Content-Disposition` header.",
    )
    buffered: bool = Field(
        default=False,
        description=(
            ...
            "\n"
            "* `false` (default): Job starts writing immediately to the download stream. The job blocks until the "
            "client downloads the data or 60 seconds elapses. Use for large files or streaming data.\n"
