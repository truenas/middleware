...
class TruecommandEntry(BaseModel):
    id: int = Field(description="Unique identifier for the TrueCommand configuration.")
    api_key: Secret[str | None] = Field(
        description="API key for authenticating with TrueCommand services. `null` if not configured.",
    )
    status: Literal[tuple(s.value for s in TruecommandStatus)] = Field(
...
class TruecommandUpdateResult(BaseModel):
    result: TruecommandEntry = Field(
        description="The updated TrueCommand configuration with current connection status.",
    )


class TruecommandConfigChangedEvent(BaseModel):
    fields: "TruecommandConfigChangedEventFields" = Field(description="Event fields.")
