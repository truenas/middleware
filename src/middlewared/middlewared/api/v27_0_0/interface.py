...
class InterfaceEntryState(BaseModel):
    ...
    fec_mode: FecModeName | None = Field(
        default=NotRequired,
        description=(
...
class InterfaceUpdate(InterfaceCreate, metaclass=ForUpdateMetaclass):
    ...
    fec_mode: FecModeName = Field(
        description=(
            "Forward Error Correction (FEC) mode. Only valid for physical interfaces.\n"
            "\n"
            "Configuring this field is only available on enterprise systems. It should be used as directed by TrueNAS "
            "Support for resolving link negotiation failures due to FEC mismatches with the upstream switch. Improper "
            "configuration of this field can cause a healthy interface to go down.\n"
            "\n"
            "* \"AUTO\": Selects the best FEC mode based on cable/port capabilities\n"
            "* \"RS\": RS-FEC (Reed-Solomon), often used for 25GbE/100GbE+ NICs\n"
            "* \"BASER\": BaseR-FEC (FireCode)\n"
...
class InterfaceAvailableFecModesArgs(BaseModel):
    id: str = Field(description="ID of the interface to query for supported FEC modes.")


class InterfaceAvailableFecModesResult(BaseModel):
    result: list[FecModeName] = Field(
        description=(
