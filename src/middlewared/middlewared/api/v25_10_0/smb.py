...
from middlewared.utils.smb import SMBSharePurpose, SMBUnixCharset, validate_smb_path_suffix
...
class SMBShareAclEntry(BaseModel):
    ...
    or name (`ae_who_str`). """
    ae_perm: Literal['FULL', 'CHANGE', 'READ'] = Field(description="Permissions granted or denied to the principal.")
...
class SmbAuditConfig(BaseModel):
    ...
    enable: bool = Field(
        default=False,
        description=(
            "Turn on auditing for the SMB share. SMB share auditing may not be enabled if `enable_smb1` is `true` in "
            "the SMB service configuration."
        ),
    )
