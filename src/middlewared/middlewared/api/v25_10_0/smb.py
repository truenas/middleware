...
class LegacyOpt(BaseModel):
    ...
    NOTE: this feature does not work with recycle bin features in client client operating systems.
...
class PrivateDatasetOpt(BaseModel):
    ...
    WARNING: ZFS dataset naming rules are more restrictive than normal path rules."""
    auto_quota: int | None = Field(default=None, examples=[10])
    """ Set the specified ZFS quota (in gibibytes) on new datasets. """
...
SmbShareOptions: Union = Annotated[
    LegacyOpt | DefaultOpt | TimeMachineOpt | MultiprotocolOpt | SMBWormOpt | PrivateDatasetOpt | ExternalOpt,
    Field()
]
...
class SmbShareEntry(BaseModel):
    ...
    purpose: Literal[
        ...
        SMBSharePurpose.NO_PRESET,
        SMBSharePurpose.TIMEMACHINE_SHARE,
        SMBSharePurpose.MULTIPROTOCOL_SHARE,
        SMBSharePurpose.MULTI_PROTOCOL_NFS,
        SMBSharePurpose.SMB_WORM_SHARE,
    ...
    available through options. Keep the default value DEFAULT_SHARE unless you have a specific reason to change it.
    ...
    NO_PRESET: This is the same as the `LEGACY_SHARE` purpose.
    ...
    through NFS, FTP, or used by containers or apps. WARNING: This setting can reduce SMB share performance because it
    turns off some SMB features for safer interoperability with external processes.

    MULTI_PROTOCOL_NFS: This is the same as the `MULTIPROTOCOL_SHARE` purpose. It is key for compatibility with older
    TrueNAS versions.

    SMB_WORM_SHARE: The SMB share makes files read-only through the SMB protocol after the set grace_period ends.
    WARNING: This setting does not work if the `path` is accessed locally or if another SMB share without the
    `SMB_WORM` preset uses it.
    ...
    """
    name: SmbShareName = Field(examples=['SHARE', 'Macrodata'])
    ...
    * The following share names are not allowed: global, printers, homes.
    """
    path: NonEmptyString | Literal['EXTERNAL'] = Field(examples=['/mnt/dozer/SHARE'])
    ...
    """ Text field that is seen next to a share when an SMB client requests a list of shares on the SMB server. """
    ro: bool = False
    ...
    browsable: bool = True
    """ If set, the share is included when an SMB client requests a list of shares on the SMB server. """
    abe: bool = False
    ...
    WARNING: this value should not be changed once data is written to the SMB share. """
    audit: SmbAuditConfig = Field(default_factory=SmbAuditConfig, example=[
    ...
    def check_purpose_options(self) -> Self:
        if self.purpose is undefined and self.options is not undefined:
            raise ValueError('You must set `purpose` if you set `options`.')

        if self.purpose is not undefined and self.options is undefined:
            raise ValueError('You must set `options` if you set `purpose`.')

        if self.options is not None:
            checked = False
            for purpose, opts in (
                (SMBSharePurpose.DEFAULT_SHARE, DefaultOpt),
                (SMBSharePurpose.LEGACY_SHARE, LegacyOpt),
                (SMBSharePurpose.NO_PRESET, LegacyOpt),
                (SMBSharePurpose.TIMEMACHINE_SHARE, TimeMachineOpt),
                (SMBSharePurpose.SMB_WORM_SHARE, SMBWormOpt),
                (SMBSharePurpose.MULTIPROTOCOL_SHARE, MultiprotocolOpt),
                (SMBSharePurpose.MULTI_PROTOCOL_NFS, MultiprotocolOpt),
                (SMBSharePurpose.EXTERNAL_SHARE, ExternalOpt),
                (SMBSharePurpose.PRIVATE_DATASETS_SHARE, PrivateDatasetOpt),
            ):
                if self.purpose != purpose:
                    continue

                if not isinstance(self.options, opts):
                    raise ValueError('Options do not match purpose.')

                checked = True
                break

            if not checked:
                raise ValueError(f'{self.purpose}: unexpected purpose.')
