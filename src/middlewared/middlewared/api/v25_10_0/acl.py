...
class FilesystemSetAclOptions(BaseModel):
    ...
    canonicalize: bool = Field(default=True, description="Whether to reorder ACL entries in Windows canonical order.")
...
class AclTemplateFormatOptions(BaseModel):
    canonicalize: bool = Field(
        default=False,
        description="Whether to normalize and canonicalize ACL entries in the response.",
    )
