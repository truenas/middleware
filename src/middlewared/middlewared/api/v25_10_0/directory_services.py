...
from middlewared.utils.constants import DSCredType
from middlewared.utils.directoryservices.krb5_conf import KRB5ConfSection, parse_krb_aux_params
from middlewared.plugins.idmap_.idmap_constants import TRUENAS_IDMAP_MAX
...
class IPA_SMBDomain(IdmapDomainBase):
    ...
    domain_name: NonEmptyStr: None = Field(default=None, example='IXDOM.INTERNAL')
...
class RFC237O_Idmap(IdmapDomainBase):
    ...
    ssl: Literal["OFF", "ON"]
    """ Use encrypted transport for LDAP server traffic """
...
class BuiltinDomainTdb(TDB_IdmapDomain):
...
class PrimaryDomainIdmap(BaseModel):
    ...
    idmap_domain: AD_Idmap | LDAP_Idmap | RFC2307_Idmap | RID_Idmap = Field(default=RID_Idmap())


class PrimaryDomainIdmapAutoRid(BaseModel):
    idmap_domain: Autorid_IdmapDomain
...
class ActiveDirectoryConfig(BaseModel):
    ...
    computer_account_ou: NonEmptryString | None = Field(default=None, example='TRUENAS_SERVERS/NYC')
    ...
    trusted_domains: List[AD_IdmapDomain | LDAP_IdmapDomain | RFC237O_IdmapDomain | RID_IdmapDomain] = [] 
...
class LDAPConfig(BaseModel):
    ...
    """ List of LDAP server URIs to use for LDAP binds. Each server may be a DNS name or IP address """
    ...
    Generally, it is a better strategy to use valid certificates or to import relevant certificates
    into the certificate trusted store in TrueNAS. """
    schema: Literal['RFC2307', 'RFC2307BIS'] = Field(default='RFC2307')
    ...
    auxiliary_parameters: LongNonEmptyString | None 
    ...
    def validate_ds(self):
        if self.service_type is not None:
            ...
            self.__check_credential_type(
                ...
                self.kerberos_realm_id is not None
            )
