...
class Autorid_Idmap(IdmapDomainBase):
    """ The AUTORID backend provides a way to use an algoritmic mapping scheme to map UIDs and GIDs to SIDs in a way
    similar to the RID backend, but automatically configures the range to be used for each domain in the forest. """
...
class RID_Idmap(IdmapDomainBase):
    ...
    master.  One way to do this is to look review the RID assigned to a recently created account in Active Directory.
    If the RID is 500000, then the range specified for this backend must contain at least 500000 unix IDs (for example
    1000000 - 2000000). """
    ...
    domain is configured in such a way that it only consumes a single SSSD idmap slice."""
...
class CredLDAPMTLS(BaseModel):
    ...
    """ Client certificate name that will be used for mutual TLS authentication to the remote LDAP server. """
...
class LDAPSearchBases(BaseModel):
    ...
    """An optional base DN to restrict LDAP searches for users. If None / null (default) then the
    `base_dn` will be used."""
    base_group: LDAP_DN | None = None
    """An optional base DN to restrict LDAP searches for groups. If None / null (default) then the
    `base_dn` will be used."""
    base_netgroup: LDAP_DN | None = None
    """An optional base DN to restrict LDAP searches for netgroups. If None / null (default) then the
    `base_dn` will be used."""
...
class LDAPMapShadow(BaseModel):
    ...
    """ This parameter contains the name of an LDAP attribute corresponding to its shadow(5) counterpart (date of the
    last password change)."""
    shadow_min: LDAP_DN | None = None
    """ This parameter contains the name of an LDAP attribute corresponding to its shadow(5) counterpart (minimum
    password age). """
    shadow_max: LDAP_DN | None = None
    """ This parameter contains the name of an LDAP attribute corresponding to its shadow(5) counterpart (maximum
    password age). """
    shadow_warning: LDAP_DN | None = None
    """ This parameter contains the name of an LDAP attribute corresponding to its shadow(5) counterpart (password
    warning period). """
    shadow_inactive: LDAP_DN | None = None
    """ This parameter contains the name of an LDAP attribute corresponding to its shadow(5) counterpart (password
    inactivity period). """
    shadow_expire: LDAP_DN | None = None
    """ This parameter contains the name of an LDAP attribute corresponding to its shadow(5) counterpart (account
    expiration date). """
...
class LDAPMapNetgroup(BaseModel):
    ...
    """ The LDAP attribute that corresponds to the netgroup's members. """
...
class LDAPConfig(BaseModel):
    ...
    """ The LDAP attribute schema type used by the remote LDAP server. """
    ...
    the specified `basedn` will be available on TrueNAS. These are only required if using a non-standard LDAP
    implementation or if the administrator only wishes to make a subset of users, groups, or netgroups available on
    TrueNAS."""
    ...
    RFC2307BIS. These are only required if using a non-standard LDAP implementaiton. """
...
class DirectoryServicesEntry(BaseModel):
    ...
    """ The pre-existing directory service type to which to bind TrueNAS. ACTIVEDIRECTORY should be selected when
    joining TrueNAS to an Active Directory domain, IPA should be selected when joining TrueNAS to a FreeIPA domain, and
    LDAP should be selected when joining to one or more OpenLDAP compatible servers."""
    credential: DSCred | None = Field(example=[
        {
    ...
    """ Credential to use for binding to the specified directory service. Kerberos credentials are required for Active
    Directory or IPA domains. There is more variety of potential authentication methods for generic LDAP environments,
    but the available authentication mechanisms depend on the how the remote LDAP server is configured.  If kerberos
    credential types are selected for the LDAP service type then GSSAPI binds will be performed in lieu of plain LDAP
    binds. Kerberos or mtls authentication should be configured when possible for improved security. """
    enable: bool
    ...
    configuration: ActiveDirectoryConfig | IPAConfig | LDAPConfig | None = Field(default=None, example=[
        {
