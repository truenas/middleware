...
class ReplicationEntry(BaseModel):
    ...
    """Whether the task will `PUSH` or `PULL` snapshots."""
    ...
    * `LOCAL` replicates to or from localhost.
    ...
    sudo: bool = False
    """`SSH` and `SSH+NETCAT` transports use sudo (which is expected to be passwordless) to run `zfs`
    ...
    """Restrict when replication task with bound periodic snapshot tasks runs. For example, you can have periodic
    snapshot tasks that run every 15 minutes, but only run replication task every hour."""
    only_matching_schedule: bool = False
    """Only replicate snapshots that match `schedule` or `restrict_schedule`."""
    allow_from_scratch: bool = False
    """Destroy all snapshots on target side and replicate everything from scratch if none of the snapshots on
    ...
    * `NONE` does not delete any snapshots.
    ...
    """Compress SSH stream. Available only for SSH transport."""
    speed_limit: int | None = Field(default=None, ge=1)
    """Limit speed of SSH stream. Available only for SSH transport."""
