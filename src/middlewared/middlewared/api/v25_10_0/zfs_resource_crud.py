...
class ZFSResourceEntry(BaseModel):
    ...
    type: Literal["ZFS_TYPE_FILESYSTEM", "ZFS_TYPE_VOLUME"]
...
class ZFSResourceQuery(BaseModel):
    ...
    query_filters: QueryFilters = Field(alias="query-filters", default=list())
    query_options: QueryOptions = Field(
        alias="query-options", default_factory=QueryOptions
    )


class ZFSResourceQueryArgs(BaseModel):
    data: ZFSResourceQuery = Field(default_factory=ZFSResourceQuery)


class ZFSResourceQueryResult(BaseModel):
    result: int | ZFSResourceEntry | list[ZFSResourceEntry]
