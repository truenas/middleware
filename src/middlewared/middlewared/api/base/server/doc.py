...
from ..jsonschema import replace_refs, clean_field_descriptions
from .api import API
...
class APIDumper:
    ...
    def _dump_method_schemas(self, method: Method):
        ...
        accepts_json_schema = clean_field_descriptions(
            replace_refs(accepts_json_schema, accepts_json_schema.get("$defs", {}))
        )

        returns_json_schema = method.methodobj.new_style_returns.model_json_schema(mode="serialization")
        returns_json_schema = clean_field_descriptions(
            replace_refs(returns_json_schema, returns_json_schema.get("$defs", {}))
        )
    ...
    def _dump_event_schemas(self, event: Event):
        ...
        for name, model in event.event["models"].items():
            ...
            schema = clean_field_descriptions(replace_refs(schema, schema.get("$defs", {})))
