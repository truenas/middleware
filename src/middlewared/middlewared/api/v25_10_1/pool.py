...
class PoolCreateTopologySpecialVdev(BaseModel):
    type: Literal["MIRROR", "STRIPE"] = Field(description="Type of special vdev configuration for metadata storage.")
    disks: list[str] = Field(description="Array of disk names to use in this special vdev.")
...
class PoolCreate(BaseModel):
    name: POOL_NAME = Field(description="Name for the new storage pool.")
...
class PoolImportPoolArgs(BaseModel):
    ...
    name: POOL_NAME | None = Field(default=None, description="If specified, import the pool using this name.")
...
class PoolValidateNameArgs(BaseModel):
    pool_name: POOL_NAME = Field(description="Pool name to validate for compliance with naming rules.")
