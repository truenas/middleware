...
class SMBShareAclEntryWhoId(BaseModel):
    id_type: Literal['USER', 'GROUP'] = Field(
        description=(
            "The type of Unix ID. If the type is `USER`, the `xid` value refers to a Unix UID. If the type is `GROUP`, "
            "the `xid` value refers to a Unix GID."
        ),
    )
    xid: int = Field(
        alias='id',
        ge=0,
        le=2147483647,
        description="Unix user ID (UID) or group ID (GID) depending on the `id_type` field.",
    )
...
class SMBShareAclEntry(BaseModel):
    ...
    ae_type: Literal['ALLOWED', 'DENIED'] = Field(
        description=(
            "The type of SMB share ACL entry. This value determines whether the permissions (ae_perm) are granted "
            "(ALLOWED) or denied (DENIED)."
        ),
    )
    ae_who_sid: SID | None = Field(default=None, description="The SID of the principal to whom this ACL entry applies.")
    ae_who_id: SMBShareAclEntryWhoId | None = Field(
        default=None,
        description="The Unix ID of the principal to whom this ACL entry applies.",
    )
    ae_who_str: NonEmptyString | None = Field(
        default=None,
        description="The User or group name of the principal to whom this ACL entry applies.",
    )
...
class SharingSMBSetaclResult(BaseModel):
    result: SMBShareAcl = Field(description="The updated SMB share ACL configuration.")


@single_argument_args('smb_getacl')
class SharingSMBGetaclArgs(BaseModel):
    share_name: NonEmptyString = Field(description="Name of the SMB share to retrieve ACL for.")
...
class SMBEntry(BaseModel):
    ...
    minimum_protocol: SMBMinProtocol = Field(
        description=(
            "Minimum SMB protocol version permitted for client connections. `SMB1` enables legacy client support (not "
            "recommended). `SMB2` is the default. `SMB3` restricts access to modern clients only. NOTE: SMB2 and SMB3 "
            "are dialects of the same protocol family; setting `SMB3` as the minimum may break access for networked "
            "embedded devices that only support SMB2."
        ),
    )
    unixcharset: SMBCharsetType = Field(
        description=(
            "Select character set for file names on local filesystem. Use this option only if you know the names are "
            "not UTF-8."
        ),
    )
    localmaster: bool = Field(
        description=(
            "When set to `true` the NetBIOS name server in TrueNAS participates in elections for the local master "
            "browser. When set to `false` the NetBIOS name server does not attempt to become a local master browser on "
            "a subnet and loses all browsing elections.\n"
            "\n"
    ...
    syslog: bool = Field(
        description=(
            ...
            "information sent to a remote syslog server. NOTE: This requires that remote syslog is globally configured "
            "on TrueNAS."
        ),
    )
    ...
    search_protocols: list[SMBSearchProtocol] = Field(
        default=[],
        description=(
            "List of enabled search protocols for the SMB server. Currently the only support search protocol is "
            "SPOTLIGHT for MacOS clients. When SPOTLIGHT search protocol support is enabled, MacOS clients can send "
            "spotlight protocol requests to the TrueNAS samba server and receive results from indexed paths."
        ),
    )
    ...
    multichannel: bool = Field(description="Enable SMB3 multi-channel support.")
    encryption: SMBEncryption = Field(
        description=(
            "SMB2/3 transport encryption setting for the TrueNAS SMB server.\n"
            "\n"
            "* `NEGOTIATE`: Enable negotiation of data encryption. Encrypt data only if the client explicitly requests "
            "it.\n"
    ...
    bindip: list[IPvAnyInterface] = Field(description="List of IP addresses used by the TrueNAS SMB server.")
    server_sid: SID | None = Field(
        description=(
            "The unique identifier for the TrueNAS SMB server. It also serves as the domain SID for all local SMB user "
            "and group accounts."
        ),
    )
    ...
    debug: bool = Field(
        description=(
            "Set SMB log levels to debug. Use this setting only when troubleshooting a specific SMB issue. Do not use "
            "it in production environments."
        ),
    )
    stateful_failover: bool = Field(
        description=(
            "Enterprise feature to ensure SMB state consistency across HA failover events. This feature is incompatible"
            " with the following share purposes: MULTIPROTOCOL_SHARE, LEGACY_SHARE. This feature is also incompatible "
            "with `minimum_protocol` set to `SMB1`."
        ),
    )
...
class SMBStatusOptions(BaseModel):
    verbose: bool = Field(default=True, description="Return verbose status output.")
    fast: bool = Field(
        default=False,
        description=(
            "Do not check if the status data is valid by checking if the processes that the status data refer to all "
            "still exist. This option decreases execution time on busy systems and clusters but may display stale data "
            "of processes that died without cleaning up properly."
        ),
    )
    restrict_user: str = Field(default='', description="Limit results to the specified user.")
    restrict_session: str = Field(default='', description="Limit results to the specified SMB session ID.")
    resolve_uids: bool = Field(
        default=True,
        description="Resolve Unix user IDs and group IDs to usernames and group names in the status output.",
    )


class SMBStatusArgs(BaseModel):
    info_level: Literal['ALL', 'SESSIONS', 'SHARES', 'LOCKS', 'BYTERANGE', 'NOTIFICATIONS'] = Field(
        default='ALL',
        description="Type of information requests. Defaults to \"ALL\".",
    )
    filters: QueryFilters = Field(default=[], description="Query filters to apply to the status results.")
    options: QueryOptions = Field(
        default=QueryOptions(),
        description="Query options for controlling result pagination and ordering.",
    )
    status_options: SMBStatusOptions = Field(
        default=SMBStatusOptions(),
        description="Additional options for retrieving and formatting SMB status information.",
    )
...
class SMBUpdateResult(BaseModel):
    result: SMBEntry = Field(description="The updated SMB service configuration.")
...
class SMBUnixcharsetChoicesResult(BaseModel):
    result: dict[str, SMBCharsetType] = Field(
        description="Available character set choices for Unix charset configuration.",
    )
...
class SMBBindipChoicesResult(BaseModel):
    result: dict[str, str] = Field(description="Available IP addresses that the SMB service can bind to.")
...
class SharingSMBPresetsResult(BaseModel):
    result: dict[str, dict] = Field(description="Available SMB share preset configurations by purpose.")


@single_argument_args('smb_share_precheck')
class SharingSMBSharePrecheckArgs(BaseModel):
    name: SmbShareName | None = Field(default=None, description="Name of the SMB share to validate (optional).")


class SharingSMBSharePrecheckResult(BaseModel):
    result: None = Field(description="Returns `null` when the SMB share configuration passes validation checks.")
...
class SmbAuditConfig(BaseModel):
    ...
    enable: bool = Field(
        default=False,
        description=(
            "Turn on auditing for the SMB share. SMB share auditing may not be enabled if `minimum_protocol` is `SMB1` "
            "in the SMB service configuration."
        ),
    )
    watch_list: list[NonEmptyString] = Field(
        default=[],
        examples=[['interns', 'contractors']],
        description="Only audit the listed group accounts. If the list is empty, all groups will be audited.",
    )
    ignore_list: list[NonEmptyString] = Field(
        default=[],
        examples=[['automation', 'apps']],
        description="List of groups that will not be audited.",
    )
...
class DefaultOpt(BaseModel):
    ...
    aapl_name_mangling: bool = Field(
        default=False,
        description=(
            "If set, illegal NTFS characters commonly used by MacOS clients are stored with their native values on the "
            "SMB server's local filesystem.\n"
            "\n"
    ...
    hostsallow: list[str] = Field(default=[], examples=[
        ...
        description=(
            "A list of IP addresses or subnets that are allowed to access the SMB share. The EXCEPT keyword may be used"
            " to limit a wildcard list.\n"
            "\n"
            "NOTE: Hostname lookups are disabled on the SMB server for performance reasons."
        ))
    hostsdeny: list[str] = Field(
        default=[],
        examples=[['150.203.4.'], ['ALL'], ['0.0.0.0/0']],
        description=(
            "A list of IP addresses or subnets that are not allowed to access the SMB share. The keyword `ALL` or the "
            "netmask `0.0.0.0/0` may be used to deny all by default."
        ),
    )
...
class LegacyOpt(BaseModel):
    ...
    recyclebin: bool = Field(
        default=False,
        description=(
            "If set, deleted files are moved to per-user subdirectories in the `.recycle` directory. The SMB server "
            "creates the `.recycle` directory at the root of the SMB share if the file is in the same ZFS dataset as "
            "the share `path`. If the file is in a child ZFS dataset, the server uses the `mountpoint` of that dataset "
            "to create the `.recycle` directory.\n"
            "\n"
    ...
    path_suffix: NonEmptyString | None = Field(
        default=None,
        examples=["%D/%U"],
        description=(
            "Path suffix template for dynamic path generation. Uses SMB variable substitution patterns like `%D` "
            "(domain) and `%U` (username)."
        ),
    )
    hostsallow: list[str] = Field(default=[], examples=[
        ['192.168.0.200', '150.203.'],
    ...
    guestok: bool = Field(
        ...
        description=(
            ...
            "\n"
            "WARNING: Additional client-side configuration downgrading security settings may be required in order to "
            "use this feature."
        ),
    )
    streams: bool = Field(
    ...
    fsrvp: bool = Field(
        default=False,
        description=(
            "If set, enable support for the File Server Remote VSS Protocol. This allows clients to manage snapshots "
    ...
    home: bool = Field(
        default=False,
        description=(
            "Use the `path` to store user home directories. Each user has a personal home directory and share. Users "
            "cannot access other user directories when connecting to shares.\n"
            "\n"
            "NOTE: This parameter changes the share `name` to `homes`. It also creates a dynamic share that mirrors the"
            " username of the user. Both shares use the same `path`. You can hide the homes share by turning off "
    ...
    acl: bool = Field(
        default=True,
        description="If set, enable mapping of local filesystem ACLs to NT ACLs for SMB clients.",
    )
    afp: bool = Field(
        default=False,
        description=(
            "If set, SMB server will read and store file metadata in an on-disk format compatible with the legacy AFP "
            "file server.\n"
            "\n"
            "WARNING: This should not be set unless the SMB server is sharing data that was originally written via the "
            "AFP protocol."
        ),
    )
    timemachine: bool = Field(
        default=False,
        description="If set, MacOS clients can use the share as a time machine target.",
    )
    timemachine_quota: int = Field(
        default=0,
        ge=0,
        le=109951162777600,
        description=(
            "If set, it defines the maximum size of a single time machine sparsebundle volume by limiting the reported "
            "disk size to the SMB client. A value of zero means no quota is applied to the share.\n"
            "\n"
            "NOTE: Modern MacOS versions you set Time Machine quotas client-side. This gives more predictable server "
            "and client behavior."
        ),
    )
    aapl_name_mangling: bool = Field(
    ...
    vuid: NonEmptyString | None = Field(
        default=None,
        examples=['d12aafdc-a7ac-4e3c-8bbd-6001f7f19819'],
        description=(
            "This value is the Time Machine volume UUID for the SMB share. The TrueNAS server uses this value in the "
            "mDNS advertisement for the Time Machine share. MacOS clients may use it to identify the volume. When you "
            "create or update a share, setting this value to null makes the TrueNAS server generate a new UUID for the "
            "share."
        ),
    )
    auxsmbconf: LongString = Field(
        default='',
        description=(
            "Additional parameters to set on the SMB share. Parameters must be separated by the new-line character.\n"
            "\n"
            "WARNING: These parameters are not validated and may cause undefined server behavior including data "
...
class TimeMachineOpt(BaseModel):
    ...
    auto_snapshot: bool = Field(
        default=False,
        description=(
            "If set, the server makes a ZFS snapshot of the share dataset when the client makes a new Time Machine "
            "backup."
        ),
    )
    auto_dataset_creation: bool = Field(
        default=False,
        description=(
    ...
    dataset_naming_schema: SmbNamingSchema | None = Field(
        default=None,
        examples=["%D/%U"],
        description=(
            "The naming schema to use when `auto_dataset_creation` is specified. If you do not set a schema, the server"
            " uses `%U` (username) if it is not joined to Active Directory. If the server is joined to Active Directory"
            " it uses `%D/%U` (domain/username). See the `VARIABLE SUBSTITUTIONS` section in the smb.conf manpage for "
            "valid strings.\n"
            "\n"
            "WARNING: ZFS dataset naming rules are more restrictive than normal path rules. For example, if `%u` is "
            "specified then the character `\\` may be inserted in the username (which is not supported in ZFS)."
        ),
    )
    vuid: NonEmptyString | None = Field(
        default=None,
...
class TimeLockedOpt(BaseModel):
    ...
    grace_period: int = Field(
        ...
        ge=60,
        le=86400 * 180,
        description="Time in seconds when write access to the file or directory is allowed.",
    )
    aapl_name_mangling: bool = Field(
        default=False,
        description=(
...
class PrivateDatasetOpt(BaseModel):
    ...
    dataset_naming_schema: SmbNamingSchema | None = Field(
        ...
        examples=["%D/%U"],
        description=(
            "The naming schema to use. If you do not set a schema, the server uses `%U` (username) if it is not joined "
            "to Active Directory. If the server is joined to Active Directory it uses `%D/%U` (domain/username).\n"
            "\n"
            "WARNING: ZFS dataset naming rules are more restrictive than normal path rules."
        ),
    )
    auto_quota: int = Field(
        default=0,
        examples=[10],
        ge=0,
        description=(
...
class FCPStorageOpt(BaseModel):
    ...
    aapl_name_mangling: Literal[True] = Field(
        default=True,
        description=(
    ...
    )  # This is just added for visibility of what feature actually does
    hostsallow: list[str] = Field(default=[], examples=[
        ['192.168.0.200', '150.203.'],
        ['150.203.15.0/255.255.255.0'],
        ['150.203. EXCEPT 150.203.6.66']
...
class SharingSMBEntry(BaseModel):
    ...
    id: int = Field(description="Unique identifier for this SMB share.")
    purpose: Literal[
        ...
        default=SMBSharePurpose.DEFAULT_SHARE.value,
        description=(
            "This parameter sets the purpose of the SMB share. It controls how the SMB share behaves and what features "
            "are available through options. The DEFAULT_SHARE setting is best for most applications, and should be "
            "used, unless there is a specific reason to change it.\n"
            "\n"
            "* `DEFAULT_SHARE`: Set the SMB share for best compatibility with common SMB clients.\n"
            "\n"
            "* `LEGACY_SHARE`: Set the SMB share for compatibility with older TrueNAS versions. Automated backend "
            "migrations use this to help the administrator move to better-supported share settings. It should not be "
            "used for new SMB shares.\n"
            "\n"
            ...
            "\n"
            "* `MULTIPROTOCOL_SHARE`: The SMB share is configured for multi-protocol access. Set this if the `path` is "
            "shared through NFS, FTP, or used by containers or apps.\n"
            ...
            "\n"
            "* `TIME_LOCKED_SHARE`: The SMB share makes files read-only through the SMB protocol after the set "
            "grace_period ends.\n"
            "  WARNING: This setting does not work if the `path` is accessed locally or if another SMB share without "
            "the `TIME_LOCKED_SHARE` purpose uses the same path.\n"
            "  WARNING: This setting might not meet regulatory requirements for write-once storage.\n"
            "\n"
            "* `PRIVATE_DATASETS_SHARE`: The server uses the specified `dataset_naming_schema` in `options` to make a "
            "new ZFS dataset when the client connects. The server uses this dataset as the share path during the SMB "
            "session.\n"
            "\n"
            "* `EXTERNAL_SHARE`: The SMB share is a DFS proxy to a share hosted on an external SMB server.\n"
            "\n"
            "* `VEEAM_REPOSITORY_SHARE`: The SMB share is a repository for Veeam Backup & Replication and supports Fast"
            " Clone.\n"
            "  NOTE: This feature is available only for TrueNAS Enterprise customers.\n"
            "\n"
            "* `FCP_SHARE`: The SMB share is a used for Final Cut Pro storage. This feature automatically configures "
            "the share to provide storage according to Apple support guidelines described in "
            "https://support.apple.com/en-ca/101919. NOTE: `aapl_extensions` must be set in the global `smb.config`. "
            "WARNING: This feature forcibly enables `aapl_name_mangling` on the SMB share which may cause unexpected "
            "behavior for data that was written without this feature enabled."
        ),
    )
    name: SmbShareName = Field(
        examples=['SHARE', 'Macrodata_refinement'],
        description=(
    ...
    path: NonEmptyString | Literal['EXTERNAL'] = Field(
        examples=['/mnt/dozer/SHARE', 'EXTERNAL'],
        description=(
    ...
    dataset: NonEmptyString | None = Field(
        examples=['dozer/SHARE'],
        description=(
            "The ZFS dataset containing this SMB share (e.g., 'tank/share'). Returns `null` for external shares or if "
            "the path cannot be resolved yet (encrypted dataset not unlocked, etc.). This is a read-only field "
            "automatically populated from \"path\"."
        ),
    )
    relative_path: str | None = Field(
        description=(
            "The path of the share relative to the dataset mountpoint (e.g., 'subfolder/data'). An empty string "
            "indicates the share is at the dataset root. Returns `null` for external shares or if the path cannot be "
            "resolved yet. This is a read-only field automatically populated from \"path\"."
        ),
    )
    enabled: bool = Field(default=True, description="If unset, the SMB share is not available over the SMB protocol.")
    comment: str = Field(
        default='',
        examples=['Mammalian nurturable'],
        description=(
    ...
    access_based_share_enumeration: bool = Field(
        default=False,
        description=(
            "If set, the share is only included when an SMB client requests a list of shares on the SMB server if the "
            "share (not filesystem) access control list (see `sharing.smb.getacl`) grants access to the user."
        ),
    )
    ...
    options: SmbShareOptions | None = Field(default=None, examples=[
        ...
        description=(
            "Additional configuration related to the configured SMB share purpose. If null, then the default options "
            "related to the share purpose will be applied."
        ))
...
class SharingSMBCreateArgs(BaseModel):
    data: SmbShareCreate = Field(description="SMB share configuration data for the new share.")


class SharingSMBCreateResult(BaseModel):
    result: SharingSMBEntry = Field(description="The created SMB share configuration.")
...
class SharingSMBUpdateArgs(BaseModel):
    id: int = Field(description="ID of the SMB share to update.")
    data: SmbShareUpdate = Field(description="Updated SMB share configuration data.")


class SharingSMBUpdateResult(BaseModel):
    result: SharingSMBEntry = Field(description="The updated SMB share configuration.")


class SharingSMBDeleteArgs(BaseModel):
    id: int = Field(description="ID of the SMB share to delete.")


class SharingSMBDeleteResult(BaseModel):
    result: Literal[True] = Field(description="Returns `true` when the SMB share is successfully deleted.")
