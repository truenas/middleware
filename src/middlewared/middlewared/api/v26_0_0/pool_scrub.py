...
class PoolScan(BaseModel):
    ...
    end_time: datetime | None = Field(description="Scan end time (`null` while the scan is still running).")
    ...
    pause: datetime | None = Field(description="Pause time of a scrub pass (`null` if the scan is not paused).")
...
class PoolScanChangedEvent(BaseModel):
    ...
    scan: PoolScan = Field(description="Scan information.")
