...
class AuthSCRAM(BaseModel):
    mechanism: Literal["SCRAM"] = Field(
        description=(
            ...
            "\n"
            "Channel binding (RFC 5929 tls-server-end-point) is supported and negotiated per "
            "exchange. A client that requires it sends a `p=tls-server-end-point` gs2 header over a "
            "TLS connection (see `rfc_str`) and the server verifies it against the certificate it "
            "serves; binding is only honored over TLS and remains optional, so clients that do not "
            "request it still authenticate.\n"
            "\n"
