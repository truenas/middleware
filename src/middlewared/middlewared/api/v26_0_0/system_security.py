...
class SystemSecurityEntry(BaseModel):
    ...
    """
    The number of days local users will have to wait before they will be allowed to change password again. One reason \
    for setting this parameter is to prevent users from bypassing password history restrictions by rapidly changing \
    their passwords. The value of None means that there is no minimum password age.
    """
    max_password_age: Annotated[int, Field(ge=7, le=365)] | None = None
    """
    The number of days after which a password is considered to be expired. After expiration no login will be possible \
    for the user. The user should contact the administrator for a password reset. The value of None means that there \
    is no maximum password age, and password aging is disabled.
    ...
    """
    The password complexity ruleset defines what character types are required for passwords used by local accounts. \
    The value of None means that there are no password complexity requirements. List items indicate a requirement for \
    at least one character in the password to be of the specified character set type.
    """
    ...
    """
    The number of password generations to keep in history for checks against password reuse for local user accounts. \
    The value of None means that history checks for password reuse are not performed.
    """
