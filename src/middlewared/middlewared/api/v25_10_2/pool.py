...
class PoolAttach(BaseModel):
    target_vdev: str = Field(
        description=(
            "The GUID of the vdev where the disk needs to be attached. In the case of a STRIPED vdev, this is the "
            "STRIPED disk GUID which will be converted into a mirror. If `target_vdev` is already a mirror, it will be "
            "converted into an n-way mirror."
        ),
    )
