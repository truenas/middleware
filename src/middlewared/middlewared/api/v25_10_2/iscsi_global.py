...
class ISCSIGlobalSessionsArgs(BaseModel):
    query_filters: QueryFilters = Field(default=[], description="Query filters to apply to the session listing.")
...
class ISCSIGlobalSessionsResult(BaseModel):
    result: list[IscsiSession] = Field(description="Array of active iSCSI sessions.")
