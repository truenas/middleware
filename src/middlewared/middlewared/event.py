...
class EventSource(object):
    ...
    def process(self):
        try:
            ...
            self.middleware.logger.warn('EventSource %r run() failed', self.name, exc_info=True)
        try:
            self.on_finish()
        except Exception:
            self.middleware.logger.warn('EventSource %r on_finish() failed', self.name, exc_info=True)
