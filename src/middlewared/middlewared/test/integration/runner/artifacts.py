...
def get_artifacts(ctx: Context) -> None:
    if ctx.verbose:
        if ctx.ha:
            get_folder(
                "/var/log", f"{ctx.artifacts}/log_nodea", "root", "testing", ctx.ip
            )
            get_folder(
                "/var/log", f"{ctx.artifacts}/log_nodeb", "root", "testing", ctx.ip2
            )
            get_cmd_result(
...
def get_folder(
    folder: str, destination: str, username: str, passwrd: str | None, host: str
) -> dict[str, bool | str]:
