...
def service_config(name: str, config: dict) -> type:
    namespace = name
...
def validate_api_method_schema_class_names(klass: type['Service']) -> None:
    """
...
class Service:
    ...
    - namespace_alias: another namespace identifier of the service, mostly used to rename and slowly deprecate old name.
    ...
    def __init_subclass__(cls, /, no_config=False):
        ...
        cls._config_specified = config_specified
        cls._config = service_config(cls.__name__, config_specified)
        cls._register_models = sum([getattr(getattr(cls, m), '_register_models', []) for m in dir(cls)], [])

        # Validate API method argument class names
        validate_api_method_schema_class_names(cls)
