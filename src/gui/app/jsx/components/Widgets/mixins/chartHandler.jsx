...
module.exports = {
  ...
  , componentDidMount: function () {
      ...
      if ( this.state.chartTypes.length > 0 ) {
        newState.graphType = _.result(
          _.findWhere( this.state.chartTypes, { primary: true } ), "type"
        );
      }
  ...
  , requestData: function () {
    ...
    _.forEach( this.state.statdResources, function ( resource ) {
      StatdMiddleware.requestWidgetData(
        resource.dataSource
        , start.format()
        ,  stop.format(), "10S"
      );
  ...
  , componentDidUpdate: function ( prevProps, prevState ) {
      var newState = {};
      if (
        this.state.statdResources.length !== prevState.statdResources.length
      ) {
        this.requestData();
        this.subscribeToUpdates();
      }
      ...
      if ( this.state.chartTypes.length !== prevState.chartTypes.length ) {
        newState.graphType = _.result(
          _.findWhere( this.state.chartTypes, { primary: true } )
        , "type" );
      }
      ...
      if (
        ...
        && this.state.statdDataLoaded
      ) {
        if ( !this.state.chart ) {
          this.drawChart();
        }
        var chartShouldReload = prevState.graphType !== this.state.graphType;
        // var statdDataExists = _.all( this.state.statdData,
        //  function( dataArray ) {
  ...
  , handleStatdChange: function () {
      ...
      if ( this.state.statdDataLoaded === true ) {
        ...
        // something in our statdResources.
        // This means the widget won't update based
        ...
        if ( updateTarget && updateTarget["variable"] ) {
          ...
          // Ideally, each of the n responses
          // will be sent one after another - if
          // they aren't, they should be queued
          // up in stagedUpdate so that they can
          // be updated as a single batch - making sure the chart
          //  only re-renders when
          // all n of the specified data is available.
          // This logic could be modified
          // to set a certain threshhold beyond which
          // the chart would force an update
          // even if it was still waiting for
          // one of the pulses, or it had receieved
          // five of one and only one of all the others, etc.

          // TODO: More clear business logic for data display

          if (
            stagedUpdate[ updateVariable ]
            && _.isArray( stagedUpdate[ updateVariable ] )
          ) {
          ...
          if (
            _.keys( stagedUpdate ).length >= this.state.statdResources.length
          ) {
            newState.statdData = {};

            _.forEach( stagedUpdate, function ( data, key ) {
              var newData = (
                this.state.statdData[ key ]
                ? this.state.statdData[ key ].concat( data )
                : []
              );
        ...
        newState.statdData = {};
        _.forEach( this.state.statdResources,  function ( resource ) {
          newState.statdData[ resource.variable ] = StatdStore.getWidgetData(
            resource.dataSource
          ) || [];

          if (
            newState.statdData[ resource.variable ]
            && newState.statdData[ resource.variable ].error
          ) {
            newState.errorMode = true;
          }
        });

        newState.statdDataLoaded = _.all(
          newState.statdData
          , function ( dataArray ) {
            return _.isArray( dataArray ) && dataArray.length > 0;
          }
      ...
      if ( newState["chart"] ) {
        ...
        if ( !_.isEmpty( newState.legendStateObj ) ) {
          if ( this.state.graphType === "pie" )	{
            newState["chart"].dispatch.changeState(
              { disabled: newState.legendStateObj }
            );
          } else {
            newState["chart"].dispatch.changeState(
              { disabled: newState.legendStateArr }
            );
          }
        ...
        // Either this is the first run,
        // the chart type has changed, or something
        // else has happened to require a complete reload of the chart.
        var graphTypeObject = _.findWhere(
        ...
        switch ( this.state.graphType ) {
          ...
          case "stacked":
            newChart = nv.models.stackedAreaChart()
              .options(
                ...
                , x                       : (
                    ...
                    graphTypeObject.x || function ( d ) { return d[0]; }
                )
                , y                       : (
                  // ...in case your data is formatted differently.
                  graphTypeObject.y || function ( d )  { return d[1]; }
                )
            ...
            // return themselves, not the parent chart,
            // so need to chain separately
          ...
          case "line":
            ...
            // when accessed directly, return themselves,
            // not the parent chart, so need to chain separately
            xLabel = graphTypeObject.xLabel || "Time";
            newChart.xAxis
              .axisLabel( xLabel )
              .tickFormat( function ( d ) {
          ...
          case "pie":
            ...
            .options(
              ...
              , x                  : (
                graphTypeObject.x || function ( d ) { return d.label; }
              )
              , y                  : (
                graphTypeObject.y || function ( d ) { return d.value; }
              )
              , color              : colors
              , showLabels         : true
              , labelThreshold     : 1

              // Configure what type of data to show in the label.
  ...
  , chartData: function ( chartType ) {
      ...
      switch ( chartType ) {
        ...
        case "pie":
          _.forEach( this.state.statdResources, function ( resource ) {
            var returnArrayMember = (
              { value : statdData[ resource.variable ][ (
                statdData[ resource.variable ].length - 1
              ) ][1]
              , label : resource.name
              }
            );
  ...
  , returnErrorMsgs: function ( resource, index ) {
      ...
      if (
        statdData[ resource.variable ]
        && statdData[ resource.variable ].msg
      ) {
        errorMsg = (
          resource.variable
          + ": "
