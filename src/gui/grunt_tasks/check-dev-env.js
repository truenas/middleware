...
module.exports = function(grunt) {
  grunt.registerTask( "check-dev-env", function() {
    ...
    if ( grunt.option( "skip-check" ) ) {
      console.log( chalk.yellow( "WARNING: Skipping environment check. I hope you know what you're doing." ) );
      asyncDone();
    }
    ...
    function isInstalled( command, callback ) {
      exec("which " + command, function(error, stdout, stderr){
        if (error){
          console.log( chalk.red( "\nERROR: Your system doesn't have " + command + " installed" ) );
    ...
    required.map( function( software ) {
      isInstalled(software, function( software, status ){
        ...
        if ( required.length === ( found.installed.length + found.missing.length ) ) {
          checkRequired();
        }
      });
    });
    ...
    function checkRequired() {
      if ( testedPlatforms.full[currentPlatform] ) {
        console.log( chalk.green( testedPlatforms.full[currentPlatform] + " is a supported development environment.") );
      } else if ( testedPlatforms.partial[currentPlatform] ) {
        console.log( chalk.yellow( testedPlatforms.partial[currentPlatform] + " is only partially supported " +
                     "as a development environment, and may not have been fully tested. Proceed with caution.") );
      } else {
        console.log( chalk.red( currentPlatform + " has not been tested as a development environment. It may " +
                     "not work as expected.") );
      }

      if ( !found.missing.length ) {
        console.log( chalk.green.bold("Development environment has all required software.\n") );
        asyncDone();
      } else {
        if ( isWindows ) {
          console.log( "Windows users will need to do the following:\n" +
                       " - Install Cygwin\n" +
          ...
          console.log( chalk.red("Cannot proceed with automatic configuration\n") );
        }
