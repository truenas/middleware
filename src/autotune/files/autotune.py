    ...
    for var, value in recommendations.items():
        ...
        if found:
            if not args.overwrite:
                ...
                continue
            elif found['type'].lower() == args.conf and found['value'] == value:
        ...
        found['var'] = var
        found['value'] = value
        found['type'] = args.conf.upper()
        found['comment'] = 'Generated by autotune'
        if foundid := found.pop('id', None):
            c.call('tunable.update', foundid, found)
        else:
            c.call('tunable.create', found)
